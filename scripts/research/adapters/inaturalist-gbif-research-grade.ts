...
const GBIF_ADAPTER_VERSION = INATURALIST_GBIF_ADAPTER_VERSION;
...
const INATURALIST_ALLOWED_LICENSES = [
...
function parseParameters(context: SourceAdapterContext): GbifAdapterParameters {
  ...
  if (parameters.basisOfRecord !== "HUMAN_OBSERVATION") {
    throw new Error("basisOfRecord must be HUMAN_OBSERVATION.");
  }
  if (parameters.occurrenceStatus !== "PRESENT") {
    throw new Error("occurrenceStatus must be PRESENT.");
  }
  if (parameters.datasetKey !== INATURALIST_GBIF_DATASET_KEY) {
    throw new Error(`datasetKey must be ${INATURALIST_GBIF_DATASET_KEY}.`);
  }
  if (typeof parameters.expectedLastParsed !== "string" || !Number.isFinite(Date.parse(parameters.expectedLastParsed))) {
    throw new Error("expectedLastParsed must be an ISO timestamp.");
  }
  if (
  ...
  return {
    ...
    expectedCrawlId: requireInteger(parameters.expectedCrawlId, "expectedCrawlId", 1, 1_000_000),
    expectedLastParsed: new Date(parameters.expectedLastParsed).toISOString(),
    maximumCoordinateUncertaintyMeters: requireInteger(
      parameters.maximumCoordinateUncertaintyMeters,
      "maximumCoordinateUncertaintyMeters",
      1,
      10_000,
    ),
...
function occurrenceSearchUrl(
  ...
  url.searchParams.set("datasetKey", parameters.datasetKey);
...
export function candidateGeography(record: GbifOccurrenceRecord): string | null {
  const parts = [record.gadm?.level2?.name, record.gadm?.level1?.name, record.countryCode].filter(Boolean);
  return parts.length > 0 ? parts.join(", ") : null;
}

function recordCountyName(record: GbifOccurrenceRecord): string | null {
  return record.gadm?.level2?.name?.trim() || null;
}

function recordStateName(record: GbifOccurrenceRecord): string | null {
  return record.gadm?.level1?.name?.trim() || record.stateProvince?.trim() || null;
}
...
export function occurrenceRejection(
  ...
  if (record.basisOfRecord !== "HUMAN_OBSERVATION") {
    return {
  ...
  if (
    canonicalText(record.stateProvince ?? "") !== canonicalText(pair.sourceStateName) ||
    canonicalText(record.gadm?.level1?.name ?? "") !== canonicalText(pair.sourceStateName)
  ) {
    return {
      reason: "outside-scope",
      notes: [
        `The record stateProvince/GADM state is ${record.stateProvince ?? "missing"}/${record.gadm?.level1?.name ?? "missing"}, not exact ${pair.sourceStateName}.`,
      ],
    };
  }
  const sourceCounty = recordCountyName(record);
  if (!sourceCounty) {
    return {
      reason: "geography-missing",
      notes: ["The GBIF interpretation does not contain a GADM level-2 county-equivalent name."],
    };
  }
  const countyResolution = resolveCountyEquivalent({
    stateCode: pair.stateCode,
    countyName: sourceCounty,
    sourceId: SOURCE_ID,
  });
  if (countyResolution.status !== "resolved") {
    return {
  ...
  if (countyResolution.county.countyFips !== pair.countyFips) {
    return {
      reason: "outside-scope",
      notes: [
        `The record GADM county ${sourceCounty} resolves to ${countyResolution.county.countyFips}, not requested ${pair.countyFips}.`,
      ],
    };
  }
  ...
  if (record.datasetKey !== parameters.datasetKey) {
    return {
      reason: "outside-scope",
      notes: [`The record dataset key is ${record.datasetKey ?? "missing"}, not ${parameters.datasetKey}.`],
    };
  }
  if (record.crawlId !== parameters.expectedCrawlId) {
    return {
      reason: "source-contradiction",
      notes: [`The record crawl ID is ${record.crawlId ?? "missing"}, not pinned crawl ${parameters.expectedCrawlId}.`],
    };
  }
  if (
    !record.lastParsed ||
    record.lastParsed.slice(0, 10) !== parameters.expectedLastParsed.slice(0, 10)
  ) {
    return {
      reason: "source-contradiction",
      notes: [`The record lastParsed date is ${record.lastParsed ?? "missing"}, not pinned date ${parameters.expectedLastParsed.slice(0, 10)}.`],
    };
  }
  if (!record.license || !parameters.allowedLicenses.includes(record.license)) {
    return {
      reason: "record-failed",
      notes: [`The record license is ${record.license ?? "missing"}, outside the retained weekly export license set.`],
    };
  }
  if (!/^https:\/\/www\.inaturalist\.org\/observations\/[0-9]+$/u.test(record.occurrenceID ?? "")) {
    return {
      reason: "record-failed",
      notes: [`The occurrence ID is not a stable iNaturalist observation URL: ${record.occurrenceID ?? "missing"}.`],
    };
  }
  if (canonicalText(record["http://unknown.org/captive_cultivated"] ?? "") !== "wild") {
    return {
      reason: "cultivated-or-captive",
      notes: [`The iNaturalist captive/cultivated field is ${record["http://unknown.org/captive_cultivated"] ?? "missing"}, not wild.`],
    };
  }
  if (
    ...
    Number(record.coordinateUncertaintyInMeters) > parameters.maximumCoordinateUncertaintyMeters
  ) {
    return {
      reason: "geography-missing",
      notes: [
        `The public coordinate or uncertainty is missing or exceeds ${parameters.maximumCoordinateUncertaintyMeters} meters.`,
      ],
    };
  }
  if (!recordDate(record)) {
    return {
      reason: "record-failed",
      notes: ["The research-grade observation lacks a usable event date."],
    };
  }
...
export function supportingPayload(
  ...
  return {
    ...
    stateProvince: recordStateName(record),
    county: recordCountyName(record),
...
export function makeGbifAssertionAndReview(
  ...
  const assertion: RunEvidenceAssertionEvent = {
    ...
    geography_match: {
      method: "Exact provider-interpreted GBIF GADM level-1 and level-2 names matched to registered state and county-equivalent identities; raw coordinates were not used by Project Isitusa",
      source_state: recordStateName(record) ?? pair.sourceStateName,
      source_county: recordCountyName(record) ?? pair.countyName,
      county_fips: pair.countyFips,
    },
    temporal_scope: sourceDate
      ? `Research-grade observation event date reported by GBIF as ${sourceDate}.`
      : "Research-grade observation with no usable event date.",
    spatial_scope: `Licensed public iNaturalist observation interpreted by GBIF within ${pair.countyLegalName}, ${pair.stateName}. This does not imply countywide abundance or current distribution.`,
    survey_scope: null,
    normalized_payload_hash: normalizedPayloadHash,
    caveats: [
      "GBIF is the provider-recommended weekly distribution path for licensed iNaturalist Research Grade observations.",
      "Research Grade, taxonomy, coordinates, and licenses can later change; this immutable run preserves the accepted record payload.",
      "This observation supports historical recorded presence only.",
  ...
  const review: EvidenceReviewEvent = {
    ...
    reason_codes: [
      ...
      "exact-taxon-match",
      "exact-county-match",
      "licensed-research-grade-occurrence-present",
    ],
    notes: [
      "The registered iNaturalist weekly-GBIF adapter publication gate permits machine-validated occurrence evidence.",
      "The record passed exact taxon, dataset snapshot, license, wild-status, date, GADM county, uncertainty, and geospatial contradiction checks.",
    ],
  };
...
async function runAdapter(context: SourceAdapterContext): Promise<SourceAdapterResult> {
  ...
  for (const speciesPairs of pairsBySpecies.values()) {
    ...
    for (const pair of speciesPairs) {
      ...
      for (const page of cachedPages) {
        for (const [recordIndex, record] of page.records.entries()) {
          if (pairAssertionIds.length > 0) continue;
          const sourceCounty = recordCountyName(record);
          if (!sourceCounty) continue;
          ...
          if (rejectionResult) {
            ...
            if (!seenPairRejectionIds.has(rejection.rejection_id) && pairRejectionIds.length < 125) {
              seenPairRejectionIds.add(rejection.rejection_id);
              rejections.push(rejection);
              pairRejectionIds.push(rejection.rejection_id);
            }
      ...
      if (!scopeComplete) {
        ...
        outcomes.push(
          makeGbifOutcome(
            ...
            speciesQueryUrls,
            [
              `Completed the registered iNaturalist weekly-GBIF screen and emitted one publishable assertion event.`,
            ],
          ),
        );
      } else {
        outcomes.push(
          makeGbifOutcome(
            ...
            speciesQueryUrls,
            [
              "Completed the declared iNaturalist weekly-GBIF query scope without publishable evidence.",
...
export const inaturalistGbifResearchGradeAdapter = adapter;
