"""Independent WI retained-source review. Reuses the pinned LA decoding scaffold; decisions are WI-specific. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-wi-original-review-20260909-r25'
ACTOR = 'ebird_monk_wi_review_r25'
BASE = 'ca7da7419e142d96edea1ce4ed7051557a0cede1'
...
SELECTED = [5515847060, 5659090167]
PAIRS = ['55025:' + SPECIES]
PINS = {
...
def review():
    ...
    require(len(scaffold_bytes) == 28724 and sha(scaffold_bytes) == 'fdbc87de145605230a8bca1938eaa4097fc9f54935ddf5222b2f2e70c65d1a01', 'job-pinned reusable decoding scaffold')
    reuse = dict(descriptor(scaffold_path, scaffold_bytes), role='Archive/hash/byte-witness decoding scaffold reused by copying standard-library code. Both WI originals and all three interpreted material rows independently inspected; no LA county decisions inherited; no MAIN code imports.')
    ...
    require(len(counties) == 1 and all(c['status'] == 'active' and c['stateName'] == 'Wisconsin' and c['stateCode'] == 'WI' for c in counties), 'active WI registry')
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            contradictions += ['unreviewed-issue/' + issue for issue in r.get('issues', []) if issue not in ['CONTINENT_DERIVED_FROM_COORDINATES', 'TAXON_CONCEPT_ID_NOT_FOUND']]
            ...
            if gd.get('level1', {}).get('name') not in [None, 'Wisconsin']:
        ...
        del data, raw
    require(len(keys) == 15000 and len(material) == 3 and sorted(selected_rows) == SELECTED, 'source/material inventory')
    ...
    for key in SELECTED:
        ...
        context_note = {
          5515847060: 'Original locality is Flight. This terse label is not proof of free flight, wildness, captivity or a particular facility. Explicit original Dane/Wisconsin matches active registry and secondary GADM. Original count is one on 2024-08-17. No specific captive statement occurs in retained fields. Same date, location and count as 5659090167 may represent the same bird/encounter; different observer and occurrence identities do not justify summing individuals.',
          5659090167: 'Original locality is Flight. It supplies no behavioral or establishment evidence. Explicit Dane/Wisconsin, positive human observation, count one and 2024-08-17 agree with the interpreted record. No specific captive statement occurs in retained fields. Shared coordinates/date with 5515847060 may indicate the same encounter despite different observer and occurrence identities.',
        }[key]
