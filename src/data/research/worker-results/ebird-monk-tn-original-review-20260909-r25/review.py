"""Independent TN retained-source review. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-tn-original-review-20260909-r25'
ACTOR = 'ebird_monk_tn_review_r25'
...
SELECTED = [5386186811, 5620289738]
PAIRS = ['47157:' + SPECIES]
PINS = {
...
def review():
    ...
    require(len(scaffold_bytes) == 28724 and sha(scaffold_bytes) == 'fdbc87de145605230a8bca1938eaa4097fc9f54935ddf5222b2f2e70c65d1a01', 'job-pinned decoder scaffold')
    provenance = dict(descriptor(scaffold_path, scaffold_bytes), role='Job-authorized independent LA decoder scaffold copied as standard-library source, not imported or executed. All TN originals, selected keys, seven interpreted rows and context conclusions independently inspected; prior LA decisions and caveats removed.')
    ...
    require(len(counties) == 1 and all(c['status'] == 'active' and c['stateName'] == 'Tennessee' and c['stateCode'] == 'TN' for c in counties), 'active TN registry')
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            checks = {
              ...
              'explicitCountry': r.get('countryCode') == 'US',
            ...
            if gd.get('level1', {}).get('name') not in [None, 'Tennessee']:
            ...
            for k in ['license', 'basisOfRecord', 'occurrenceStatus', 'taxonomicStatus', 'individualCount', 'coordinateUncertaintyInMeters', 'locality', 'eventDate', 'establishmentMeans', 'occurrenceRemarks', 'http://unknown.org/status', 'http://unknown.org/crawl_attempt', 'http://unknown.org/omitFromScheduledCrawl']:
            ...
            for classification in r.get('classifications', {}).values():
                for issue in classification.get('issues', []):
                    distributions['classificationIssueOccurrences'][issue] += 1
            for issue in r.get('issues', []):
                distributions['issues'][issue] += 1
            if r['key'] in SELECTED:
                selected_rows[r['key']] = (r, entry)
        del data, raw
    require(len(keys) == 15000 and len(material) == 7 and sorted(selected_rows) == SELECTED, 'source/material inventory')
    ...
    for key in SELECTED:
        ...
        context_note = {
          5386186811: 'Original July 29, 2024 record explicitly identifies Shelby, Tennessee, one Monk Parakeet at Shelby Farms Park--Gardner Rd. Marsh. Secondary GADM agrees. No captive, pet, cage, aviary, establishment or escape assertion appears. Another interpreted-only July 29 row at the same site has a different occurrence identity; it may concern the same bird and is not an original approval.',
          5620289738: 'Original July 31, 2024 record explicitly identifies Shelby, Tennessee, one Monk Parakeet at Shelby Farms Park--Gardner Rd. Marsh. The source date, count, identity and coordinates agree with interpretation. No specific captivity statement appears. Same-site observations on different dates do not establish independent birds, establishment or current persistence.',
        }[key]
