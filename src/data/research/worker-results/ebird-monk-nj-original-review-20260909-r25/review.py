"""NJ retained-source review with disclosed LA scaffold reuse; independent NJ decisions. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-nj-original-review-20260909-r25'
ACTOR = 'ebird_monk_nj_review_r25'
...
SELECTED = [5337715259, 5376017540, 5377761044, 5424850761, 5437174669]
PAIRS = ['34003:' + SPECIES, '34013:' + SPECIES, '34017:' + SPECIES]
SCAFFOLD = {'path': 'src/data/research/worker-results/ebird-monk-la-original-review-20260909-r24/review.py', 'bytes': 28724, 'sha256': 'fdbc87de145605230a8bca1938eaa4097fc9f54935ddf5222b2f2e70c65d1a01'}
PINS = {
...
def review():
    sb = (ROOT / SCAFFOLD['path']).read_bytes()
    require(len(sb) == SCAFFOLD['bytes'] and sha(sb) == SCAFFOLD['sha256'], 'reused scaffold pin')
    blobs = {}; inventory = []; inputs = [SCAFFOLD.copy()]; docs = []; evaluations = {}
    ...
    require(len(counties) == 3 and all(c['status'] == 'active' and c['stateName'] == 'New Jersey' and c['stateCode'] == 'NJ' for c in counties), 'active NJ registry')
    ...
    context_pattern = re.compile(r'\b(home|house|apartment|yard|garden|zoo|private|captive|captivity|caged|aviary|pet|escape[de]?|feral|colony|nest|stables|refuge)\b', re.I)
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            if gd.get('level1', {}).get('name') not in [None, 'New Jersey']:
        ...
        del data, raw
    require(len(keys) == 15000 and len(material) == 315 and sorted(selected_rows) == SELECTED, 'source/material inventory')
    ...
    for key in SELECTED:
        ...
        checks = {'exactOriginalGeography': original_pair == s['pair'] == entry['pairKey'], 'exactOriginalTaxon': f('scientificName') == NAME and f('genus') == 'Myiopsitta' and f('specificEpithet') == 'monachus', 'positiveOriginalObservation': f('basisOfRecord') == 'HumanObservation' and f('occurrenceStatus') == 'PRESENT' and (f('individualCount') is None or int(f('individualCount')) > 0), 'originalDate': date(int(f('year')), int(f('month')), int(f('day'))).isoformat() == r['eventDate'], 'originalStableIdentity': original.get(GBIF + 'gbifID') == str(key) and f('occurrenceID') == r['occurrenceID'] and original.get(DC + 'identifier') == r['identifier'], 'originalPublisher': original.get('publishingOrgKey') == PUBLISHER and f('institutionCode') == 'CLO' and f('collectionCode') == 'EBIRD' and f('country') == 'United States', 'sourceLevelLicenseSupported': metadata['license'] == r['license'] == LICENSE}
        ...
        for field in ['decimalLatitude', 'decimalLongitude', 'individualCount', 'year', 'month', 'day']:
            checks['originalInterpreted/' + field] = (f(field) is None and r.get(field) is None) or (f(field) is not None and r.get(field) is not None and float(f(field)) == float(r[field]))
        ...
        context_note = {
          5337715259: 'Mill Creek Point in the Meadowlands locality has explicit original Hudson county, corroborated by secondary GADM. One individual observed on 2024-09-26. No captivity or contradictory location statement in original fields.',
          5376017540: 'I-78 W, Newark locality has explicit original Essex county and agreeing secondary GADM. Longitude printed in locality is rounded relative to the original structured coordinate, not a location contradiction. Two individuals observed on 2024-05-21. This is the only retained Essex row, not complete Essex research.',
          5377761044: 'Mill Creek Marsh in the Meadowlands locality has explicit original Hudson county, corroborated by secondary GADM. One individual observed on 2024-09-27. Same observer as the selected Point record on the preceding day; distinct records do not prove independent bird groups or justify adding counts.',
          5424850761: 'Overpeck County Park--Stables/Wildlife Refuge is a locality name, not evidence that the parakeet was captive. Explicit original Bergen county agrees with secondary GADM. Original and interpreted individualCount are both absent; PRESENT and HumanObservation explicitly support occurrence without a numeric abundance claim.',
          5437174669: 'Overpeck County Park original Bergen county agrees with secondary GADM. Original and interpreted individualCount are both absent; PRESENT and HumanObservation explicitly support occurrence without a numeric abundance claim. No specific captivity statement is present.',
        }[key]
