"""Independent NM retained-source review; job-pinned LA decoding scaffold reuse disclosed. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-nm-original-review-20260909-r25'
ACTOR = 'ebird_monk_nm_review_r25'
BASE = '4373b154c43c5fe216cd58155799c6512a3e0af1'
...
SELECTED = [5339823019, 5490028117]
PAIRS = ['35053:' + SPECIES]
PINS = {
...
def review():
    ...
    require(len(scaffold_bytes) == 28724 and sha(scaffold_bytes) == 'fdbc87de145605230a8bca1938eaa4097fc9f54935ddf5222b2f2e70c65d1a01', 'job-pinned scaffold')
    reuse = dict(descriptor(scaffold_path, scaffold_bytes), role='Source-decoding scaffold reused with disclosure; new NM original bytes and all 13 material rows independently inspected. No LA county decisions inherited; no MAIN code imports.')
    blobs = {}; inventory = []; inputs = [descriptor(scaffold_path, scaffold_bytes)]; docs = []; evaluations = {}
    ...
    require(len(counties) == 1 and all(c['status'] == 'active' and c['stateName'] == 'New Mexico' and c['stateCode'] == 'NM' for c in counties), 'active NM registry')
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            if gd.get('level1', {}).get('name') not in [None, 'New Mexico']:
        ...
        del data, raw
    require(len(keys) == 15000 and len(material) == 13 and sorted(selected_rows) == SELECTED, 'source/material inventory')
    ...
    for key in SELECTED:
        ...
        context_note = {
          5339823019: 'Original records two birds on 2024-09-25 at Walmart Parking Lot, Socorro, NM. Explicit Socorro county and New Mexico state match the active registry and secondary GADM. Locality is consistent with that geography; no explicit captive, cage, pet or exhibit statement exists in the original. This supports a historical positive witness only.',
          5490028117: 'Original records four birds on 2024-09-17 at Socorro: Walmart Parking Lot. Original taxon, county, state, date and source identity agree with the interpreted record. This is a distinct source record on an earlier date, not proof of different individual birds, independent population size, establishment or current persistence. No specific captivity contradiction appears in the retained original.',
        }[key]
