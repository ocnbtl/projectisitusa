"""Independent WV retained-source review. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-wv-original-review-20260909-r25'
ACTOR = 'ebird_monk_wv_review_r25'
...
def review():
    ...
    require(len(counties) == 1 and all(c['status'] == 'active' and c['stateName'] == 'West Virginia' and c['stateCode'] == 'WV' for c in counties), 'active WV registry')
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            if gd.get('level1', {}).get('name') not in [None, 'West Virginia']:
    ...
    for key in SELECTED:
        ...
        context_note = 'Original locality is Stuart Recreation Area. This is a location label, not an explicit captivity or release statement. The original explicitly names Randolph, West Virginia, corroborated by secondary GADM. Original observation date is 2024-08-05 with count 1 and HumanObservation/PRESENT; identity OBS2367122160 and coordinates agree with the interpreted row. No explicit captive, cage, exhibit, escape, or pet context is present. This supports only an individually reviewed historical recorded occurrence, not establishment, current persistence, or independent expert approval.'
