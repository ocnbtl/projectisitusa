"""Independent IN retained-source review. Reuses hash-pinned LA decoding scaffold with fresh IN decisions. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-in-original-review-20260909-r25'
ACTOR = 'ebird_monk_in_review_r25'
...
SELECTED = [5430428172, 5465668478, 5473776059]
PAIRS = ['18089:' + SPECIES, '18091:' + SPECIES]
PINS = {
...
def review():
    ...
    require(len(counties) == 2 and all(c['status'] == 'active' and c['stateName'] == 'Indiana' and c['stateCode'] == 'IN' for c in counties), 'active IN registry')
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            if gd.get('level1', {}).get('name') not in [None, 'Indiana']:
        ...
        del data, raw
    require(len(keys) == 15000 and len(material) == 41 and sorted(selected_rows) == SELECTED, 'source/material inventory')
    ...
    for key in SELECTED:
        ...
        context_note = {
          5430428172: 'Lakefront Park and Sanctuary, Hammond is a locality label; sanctuary alone does not establish captivity. Original explicitly gives Lake, Indiana, one bird on 2024-09-24, HumanObservation and PRESENT. Secondary GADM agrees. No captive, caged, pet or exhibit assertion appears in the retained original.',
          5465668478: 'NIPSCO Griffith Power Substation is an infrastructure locality, without captive or exhibit context. Original explicitly gives Lake, Indiana, five birds on 2024-07-28, HumanObservation and PRESENT. Secondary GADM agrees. Other interpreted substation observations may repeat encounters and are not independent abundance samples.',
          5473776059: 'The La Porte street address is a locality, not evidence of captive birds. Explicit original county LaPorte maps exactly to active 18091; secondary GADM agrees. The locality latitude is rounded compared with the structured latitude, not a material contradiction. Original explicitly records one bird on 2024-09-28, HumanObservation and PRESENT. No explicit captive context appears.',
        }[key]
    ...
    reuse_path = 'src/data/research/worker-results/ebird-monk-la-original-review-20260909-r24/review.py'
    reuse_bytes = (ROOT / reuse_path).read_bytes()
    require(len(reuse_bytes) == 28724 and sha(reuse_bytes) == 'fdbc87de145605230a8bca1938eaa4097fc9f54935ddf5222b2f2e70c65d1a01', 'reused decoder pin')
    inputs.append(descriptor(reuse_path, reuse_bytes))
    return {
      'schemaVersion': 1, 'kind': 'independent-offline-original-record-method-proposal',
      'jobId': JOB, 'leaseId': 'lease-' + JOB + '-1', 'actorId': ACTOR, 'baseSha': BASE,
      ...
      'sourceId': 'gbif-ebird', 'stateCode': 'IN', 'speciesId': SPECIES, 'sourceYear': 2024, 'scopeComplete': False,
      'metrics': {'baseline': metrics.copy(), 'final': metrics.copy(), 'net': metrics.copy()},
      'reviewProvenance': {
        'reusedScaffold': descriptor(reuse_path, reuse_bytes),
        'reuseScope': 'Standard-library archive decoding, hash checks, byte witnesses and offline reproduction structure. No imported MAIN code and no inherited Louisiana county decisions.',
        'independentInspection': 'Inspected the complete original response fields for all three leased keys and the date, locality, geography, issues and semantic context of all 41 Indiana interpreted rows before authoring decisions.',
        'preflight': {'ok': True, 'head': BASE, 'command': 'node .agents/skills/isitusa-evidence-worker/scripts/validate-worker.mjs preflight --lease C:/Code/project-isitusa/.cache/research/campaigns/20260908-r19-ebird-bulk/in-r25-lease.json --repo C:/Code/project-isitusa-worktrees/' + JOB},
        'documentationPrecedence': 'Current AGENTS and successful frozen lease pins supersede historical architecture/workflow skill versions. Some general orchestration and historical documentation paths are absent from this sparse leased worktree; exact job, lease and frozen evidence contracts were read.'
      },
      'inputDescriptors': inputs, 'archiveMemberDescriptors': inventory, 'verifiedDescriptorReferences': checked_refs,
      'archiveVerification': {'outerArchives': 4, 'embeddedMemberOccurrences': len(inventory), 'uniqueEmbeddedPaths': len(blobs), 'allEmbeddedStoredAndDecodedHashes': True, 'retainedDescriptorReferencesChecked': len(checked_refs), 'sourcePagesProcessedSequentially': 50},
      ...
      'geography': {'registryEntries': counties, 'method': 'Exact original state and unique active county name or registered alias; no coordinate routing.', 'secondaryGadmLabelVariationKeys': sorted(raw_gadm_variations), 'secondaryGadmConflicts': sorted(secondary_conflicts, key=lambda x: x['key'])},
      'sourceMethodReview': {
        'authority': 'Cornell Lab of Ornithology via GBIF', 'datasetMetadataPath': metadata_path, 'datasetDoi': metadata['doi'], 'datasetLicense': metadata['license'], 'datasetTemporalCoverages': metadata['temporalCoverages'], 'taxonomyResponsePath': taxpath, 'taxonomyResponse': taxonomy, 'datasetMachineTags': metadata['machineTags'],
        'unknownFieldQualification': 'Original http://unknown.org/status=not_reviewed and crawler fields match older dataset-level machine tag values. Matching values provide dataset-context inference, not proof of per-record field provenance, individual rejection, human review or individual expert approval.',
        'contextEvidence': context_evidence, 'existingRegistryAdapter': source['researchAdapter'],
        'independentMethodProposal': 'MAIN must register and test a selected-original witness profile. Pin exact selected keys before requests, verify complete selected-original acquisition separately from incomplete parent search, retain interpreted conflicts and individually held originals, and accept only exact positive original identity/date/taxon/county records with source license and qualified context. Existing v2 replay is not approval for deferred pairs.',
        'requiredMainTests': ['Missing/changed original bytes or decoded hash fails', 'Extra/missing selected key or selected locator mismatch fails', 'Original county/state/taxon/date/identity or coordinate contradiction holds that original', 'Specific captivity context holds that original without discarding another valid record', 'Home/private/zoo/sanctuary locality alone does not infer captivity', 'Opaque dataset machine tags cannot create human-approved review', 'Incomplete parent pagination never becomes completed source screen or negative outcome', 'Unacquired interpreted originals never enter approved original recordKeys']
      },
      'originalReviews': original_reviews,
      'materialReview': {
        'rows': len(material), 'selectedOriginalRows': 3, 'otherInterpretedRowsOriginalsNotAcquired': 38,
        'pairs': [{'pairKey': pair, 'rowCount': len(pair_rows[pair]), 'allInterpretedKeys': sorted(x['key'] for x in pair_rows[pair])} for pair in PAIRS],
        'fieldAndIssueCounts': {k: dict(sorted(v.items())) for k, v in sorted(distributions.items())},
        'contextRows': sorted(context_rows, key=lambda x: x['key']),
        'rowsWithLocators': sorted(material, key=lambda x: (x['pairKey'], x['key'])),
        'independentContextFindings': ['Lake has 40 retained interpreted rows and LaPorte has one. All have positive counts and agree on accepted species, explicit county, state and secondary GADM.', 'Many Lake rows share the NIPSCO locality and dates; distinct stable occurrence identifiers do not prove independent encounters and counts must not be summed into abundance.', 'Monk Parakeet Nest and monk tower are locality names on unacquired originals. They do not establish breeding or establishment.', 'Street addresses, infrastructure and sanctuary locality labels alone do not establish captive context. No explicit captivity text appears in retained semantic fields.']
      },
      'caveats': [
        'Proposal only: no immutable completed evidence-run manifest and no canonical writes.',
        'Three selected originals are supported; 38 other material rows remain interpreted-only, with no original approval. No retained contradiction was found in these rows.',
        'CONTINENT_DERIVED_FROM_COORDINATES annotates continent only; county authority is the original publisher county. TAXON_CONCEPT_ID_NOT_FOUND leaves the Avibase concept unresolved while exact accepted GBIF species classifications agree.',
        'Missing uncertainty and photographs alone do not invalidate explicit original county authority. These historical point observations do not establish countywide abundance or current persistence.',
        'eBird rules favor wild observations, but dataset context is not individual proof of wildness or expert approval. No explicit captive contradiction was found in the three selected original responses.',
        'No fresh retrieval, source-screen completion, establishment, breeding, invasive impact, absence or non-detection claim is made.'
      ],
