"""Independent SC retained-source review; reuses the job-pinned LA decoding scaffold with explicit provenance. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-sc-original-review-20260909-r25'
ACTOR = 'ebird_monk_sc_review_r25'
...
REUSE_HASH = 'fdbc87de145605230a8bca1938eaa4097fc9f54935ddf5222b2f2e70c65d1a01'
PINS = {
...
def review():
    ...
    require(len(reuse_bytes) == 28724 and sha(reuse_bytes) == REUSE_HASH, 'job-pinned decoder scaffold')
    ...
    require(len(counties) == 1 and all(c['status'] == 'active' and c['stateName'] == 'South Carolina' and c['stateCode'] == 'SC' for c in counties), 'active SC registry')
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            if gd.get('level1', {}).get('name') not in [None, 'South Carolina']:
        ...
        del data, raw
    require(len(keys) == 15000 and len(material) == 1 and sorted(selected_rows) == SELECTED, 'source/material inventory')
    ...
    for key in SELECTED:
        ...
        context_note = 'Independently inspected original 5354519684 and matching page-048 row 14: West Ashley Park locality; explicit Charleston, South Carolina county/state matches the active registry and secondary GADM. Count 1 on 2024-09-21, exact Myiopsitta monachus, original HumanObservation/PRESENT, stable OBS2478413103 identity and coordinates agree. No occurrenceRemarks, eventRemarks, habitat, establishment, captive, cage or exhibit assertion occurs in the retained original. Park locality alone does not establish individual wildness; dataset rules supply qualified context only.'
