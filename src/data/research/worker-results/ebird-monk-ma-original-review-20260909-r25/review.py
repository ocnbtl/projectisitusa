"""Independent MA record decisions using the disclosed LA source-decoding scaffold. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-ma-original-review-20260909-r25'
ACTOR = 'ebird_monk_ma_review_r25'
...
def review():
    ...
    blobs = {}; inventory = []; inputs = [descriptor(SCAFFOLD, scaffold_bytes)]; docs = []; evaluations = {}
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            if secondary is None:
                ...
                secondary_conflicts.append({'key': r['key'], 'pairKey': pair, 'publisherCounty': r['county'], 'secondaryGadm': gd, 'locator': locator, 'disposition': 'held-selected-original' if r['key'] in SELECTED else 'held-interpreted-only-original-not-acquired'})
    ...
    for key in SELECTED:
        ...
        context_note = {
          ...
          5344669437: 'Held: the original explicitly reports Norfolk/Massachusetts and the locality names Dedham, but interpreted secondary GADM reports Suffolk. Original and interpreted coordinates agree; no approved coordinate routing or boundary adjudication is supplied. Preserve the explicit original county, do not reroute to Suffolk, and require MAIN geography review before approval. A street address alone does not imply captivity.',
    ...
    return {'schemaVersion': 1, 'kind': 'independent-offline-original-record-method-proposal', 'jobId': JOB, 'leaseId': 'lease-' + JOB + '-1', 'actorId': ACTOR, 'baseSha': BASE, 'status': 'proposal-ready-main-method-required', 'adapterReview': {'actorId': ACTOR, 'baseSha': BASE, 'status': 'proposal-ready-main-method-required', 'pairs': adapter_pairs}, 'provenance': {'scaffold': descriptor(SCAFFOLD, scaffold_bytes), 'reusedPortions': ['standard-library gzip/base64 and JSON archive decoding', 'outer and embedded descriptor/hash verification', 'byte-field lexeme witnesses', 'sequential source-page iteration and structured check framework', 'offline deterministic serializer/comparator and Windows peak working-set measurement'], 'independentWork': ['fresh inspection of all four Massachusetts original values and all 34 material interpreted rows', 'new state/pair/key/count scope and per-record context decisions', 'distinction between seven missing GADM values and the actual Norfolk/Suffolk disagreement', 'new adapterReview support and hold dispositions'], 'limitation': 'This is independent record review using a previously verified independent-worker scaffold; it is not an independently invented parser and inherits no Louisiana record conclusions.'}, 'sourceId': 'gbif-ebird', 'stateCode': 'MA', 'speciesId': SPECIES, 'sourceYear': 2024, 'scopeComplete': False, 'metrics': {'baseline': metrics.copy(), 'final': metrics.copy(), 'net': metrics.copy()}, 'inputDescriptors': inputs, 'archiveMemberDescriptors': inventory, 'verifiedDescriptorReferences': checked_refs, 'archiveVerification': {'outerArchives': 4, 'embeddedMemberOccurrences': len(inventory), 'uniqueEmbeddedPaths': len(blobs), 'allEmbeddedStoredAndDecodedHashes': True, 'retainedDescriptorReferencesChecked': len(checked_refs), 'sourcePagesProcessedSequentially': 50}, 'originalAcquisitionLineage': {'receipt': descriptor(receipt_path, rb), 'repositoryBaseCommit': receipt['repositoryBaseCommit'], 'recipe': receipt['recipe'], 'parentDiagnostic': receipt['parentDiagnostic'], 'parentSourceReceipts': receipt['parentSourceReceipts'], 'startedAt': receipt['startedAt'], 'finishedAt': receipt['finishedAt'], 'allSelectedOriginalsCompleted': 141, 'allSelectedOriginalFailures': 0, 'automaticRetries': 0, 'selectedForThisLease': 4}, 'parentSearch': {'receivedRows': 15000, 'declaredRows': 21878, 'remainingUnretrievedRows': 6878, 'paginationComplete': False, 'terminalPages': 0, 'frozenProviderSnapshot': False, 'nationalKeysVerifiedUnique': len(keys), 'nationalOccurrenceIdsVerifiedUnique': len(ids), 'qualification': 'Retained positive witnesses can support individual historical occurrences. Partial search cannot complete a source-family screen or support any negative claim.'}, 'geography': {'registryEntries': counties, 'method': 'Exact original state and unique active county name or registered alias; no coordinate routing.', 'missingSecondaryGadmKeys': sorted(missing_gadm_keys), 'missingSecondaryGadmInterpretation': 'Seven interpreted Suffolk rows have empty GADM. Missing secondary geography is not an explicit contradiction and does not invalidate publisher county text. Their originals remain unacquired.', 'secondaryGadmConflicts': sorted(secondary_conflicts, key=lambda x: x['key'])}, 'sourceMethodReview': {'authority': 'Cornell Lab of Ornithology via GBIF', 'datasetMetadataPath': metadata_path, 'datasetDoi': metadata['doi'], 'datasetLicense': metadata['license'], 'datasetTemporalCoverages': metadata['temporalCoverages'], 'taxonomyResponsePath': taxpath, 'taxonomyResponse': taxonomy, 'datasetMachineTags': metadata['machineTags'], 'unknownFieldQualification': 'Original http://unknown.org/status=not_reviewed and crawler fields match older dataset-level machine tag values. Matching values provide dataset-context inference, not proof of per-record field provenance, individual rejection, human review or individual expert approval.', 'contextEvidence': context_evidence, 'existingRegistryAdapter': source['researchAdapter'], 'independentMethodProposal': 'MAIN must register and test a selected-original witness profile for this independently reviewed scope. Pin exact selected keys before requests, verify complete selected-original acquisition separately from incomplete parent search, retain all interpreted conflicts and individually held originals, and accept only exact positive original identity/date/taxon/county records with source license and qualified context. Do not reuse the existing v2 replay as approval for these deferred pairs.', 'requiredMainTests': ['Missing/changed original bytes or decoded hash fails', 'Extra/missing selected key or selected locator mismatch fails', 'Original county/state/taxon/date/identity or coordinate contradiction holds that original', 'Specific captivity context holds that original without discarding another valid record', 'Home/private/zoo locality alone does not infer captivity', 'Opaque dataset machine tags cannot create human-approved review', 'Incomplete parent pagination never becomes completed source screen or negative outcome', 'Unacquired interpreted originals never enter approved original recordKeys']}, 'originalReviews': original_reviews, 'materialReview': {'rows': len(material), 'selectedOriginalRows': 4, 'otherInterpretedRowsOriginalsNotAcquired': 30, 'pairs': [{'pairKey': pair, 'rowCount': len(pair_rows[pair]), 'allInterpretedKeys': sorted(x['key'] for x in pair_rows[pair])} for pair in PAIRS], 'fieldAndIssueCounts': {k: dict(sorted(v.items())) for k, v in sorted(distributions.items())}, 'contextRows': sorted(context_rows, key=lambda x: x['key']), 'rowsWithLocators': sorted(material, key=lambda x: (x['pairKey'], x['key']))},  'caveats': [
      ...
      'Four selected originals reviewed: three supported across Nantucket and Suffolk; Norfolk key 5344669437 held for explicit publisher county versus secondary GADM disagreement. This is a hold pending resolution, not a finding that the publisher county is wrong.',
