"""Independent LA retained-source review. Standard library only; no MAIN adapter imports.
...
JOB = 'ebird-monk-la-original-review-20260909-r24'
ACTOR = 'ebird_monk_la_review_r24'
...
SELECTED = [5344226933, 5349939442, 5371556583, 5393086856, 5439353964, 5464004981]
PAIRS = ['22051:' + SPECIES, '22071:' + SPECIES, '22087:' + SPECIES]
PINS = {
...
def review():
    ...
    require(len(counties) == 3 and all(c['status'] == 'active' and c['stateName'] == 'Louisiana' and c['stateCode'] == 'LA' for c in counties), 'active LA registry')
    ...
    for page in pages:
        ...
        for ix, r in enumerate(data['results']):
            ...
            if secondary != r.get('county'):
                if r.get('county') == 'St. Bernard' and secondary == 'Saint Bernard':
                    raw_gadm_variations.append(r['key'])
                else:
            ...
            if gd.get('level1', {}).get('name') not in [None, 'Louisiana']:
        ...
        del data, raw
    require(len(keys) == 15000 and len(material) == 436 and sorted(selected_rows) == SELECTED, 'source/material inventory')
    ...
    for key in SELECTED:
        ...
        context_note = {
          5349939442: 'New Orleans appears as a locality city label, not a conflicting original county assertion. Explicit Jefferson maps exactly and secondary GADM also says Jefferson. No specific captivity context.',
          5439353964: 'River batture locality, positive count and original human observation. No specific captivity context.',
          5371556583: 'University campus locality does not imply captivity. No cage, captive or exhibit statement in original record.',
          5464004981: 'St Bernard Ave is a street name in locality text, not a county assertion. Explicit original county is Orleans, corroborated by secondary GADM. No specific captivity context.',
          5344226933: 'Private locality does not imply captive birds. Two selected records share date, coordinates and count but have different occurrence IDs and recordedBy identities; retain both without claiming independent encounters or summing birds.',
          5393086856: 'Private locality does not imply captive birds. This may repeat the same bird group as the other selected St. Bernard record; distinct source identities do not prove independent encounters.',
        }[key]
        original_reviews.append({'key': key, 'pairKey': entry['pairKey'], 'disposition': 'held' if reasons else 'supported', 'reasonCodes': reasons or ['exact-original-taxon', 'exact-original-active-parish', 'positive-original-human-observation', 'identity-date-coordinate-agreement', 'no-specific-captivity-contradiction-in-retained-record'], 'checks': checks, 'originalResponse': response, 'interpretedLocator': entry['locator'], 'selectionLineage': s, 'originalFields': original, 'originalBytesBase64': base64.b64encode(raw).decode(), 'originalFieldByteWitnesses': byte_fields(raw, original), 'interpretedClassification': r['classifications'], 'secondaryGadm': r.get('gadm'), 'originalLicenseFieldPresent': DWC + 'license' in original or DC + 'license' in original, 'licenseInference': 'CC BY 4.0 is explicit in dataset metadata and interpreted record; no license field is fabricated in the original response.', 'contextReview': context_note})
    for pair in PAIRS:
