import { EBIRD_SELECTED_VERSION, EBIRD_SELECTED_MODE, buildEbirdSelectedResult } from "./ebird-selected-original-observations";
...
import {
  ...
  USGS_BBS_ROUTE_START_GEOGRAPHY_POLICY,
  USGS_BBS_ROUTE_START_SOURCE_ID,
...
export function isCommittedSnapshotReplayReceipt(
  ...
  if (receipt.source_id === EBIRD_SOURCE && receipt.adapter_id === EBIRD_ADAPTER && receipt.adapter_version === EBIRD_SELECTED_VERSION
    && receipt.parameters.mode === EBIRD_SELECTED_MODE && typeof receipt.parameters.methodReviewSha256 === "string"
    && /^[a-f0-9]{64}$/u.test(receipt.parameters.methodReviewSha256) && typeof receipt.parameters.methodReviewPath === "string"
    && /^src\/data\/research\/source-method-reviews\/ebird-selected-original-[a-z0-9-]+\.json$/u.test(receipt.parameters.methodReviewPath)) return true;
  ...
  if (receipt.source_id === WQP_SOURCE && receipt.adapter_id === WQP_ADAPTER && receipt.adapter_version === WQP_FISH_VERSION
    ...
    && /^[a-f0-9]{64}$/u.test(receipt.parameters.methodReviewSha256)) return true;
  if (receipt.source_id === WQP_SOURCE && receipt.adapter_id === WQP_ADAPTER && receipt.adapter_version === WQP_REVIEWED_FISH_VERSION
    && receipt.parameters.mode === "retained-reviewed-fish-measurement" && typeof receipt.parameters.methodReviewSha256 === "string"
    && /^[a-f0-9]{64}$/u.test(receipt.parameters.methodReviewSha256) && typeof receipt.parameters.methodReviewPath === "string"
    && /^src\/data\/research\/source-method-reviews\/wqp-reviewed-fish-[a-z0-9-]+\.json$/u.test(receipt.parameters.methodReviewPath)) return true;
...
export function validateResearchRunInMemory(input: {
  ...
  if (sourceId === HONEY_POSITIVE_SOURCE && (receipt.adapter_version === HONEY_POSITIVE_VERSION || receipt.parameters.mode === "retained-positive-survey")) {
    assert(receipt.adapter_id === HONEY_POSITIVE_ADAPTER && receipt.adapter_version === HONEY_POSITIVE_VERSION, "Wrong honey bee positive adapter version.");
    const expected = buildHoneyPositiveResult({runId, sourceId, stateCode, runStartedAt: receipt.started_at, parameters: receipt.parameters,
      ...
      p => readCommittedBytes(root, receipt.code_commit, p));
    for (const field of ["assertions", "reviews", "rejections", "outcomes", "upstreamRequests"] as const)
      assert(stableJson(result[field]) === stableJson(expected[field]), "Honey bee positive " + field + " differ from canonical retained-source reconstruction.");
    assert(result.candidateRecordCount === expected.candidateRecordCount && result.duplicateRecordCount === expected.duplicateRecordCount
      && stableJson(result.errors) === stableJson(expected.errors) && stableJson(result.warnings) === stableJson(expected.warnings), "Honey bee canonical counts or diagnostics differ.");
    assert(receipt.artifacts.length === expected.artifacts.length, "Honey bee witness count differs.");
  ...
  if (sourceId === EBIRD_SOURCE) {
    assert(receipt.adapter_id === EBIRD_ADAPTER && [EBIRD_VERSION, EBIRD_SELECTED_VERSION].includes(receipt.adapter_version), "Wrong EOD original-field adapter version.");
    const reconstruct = receipt.adapter_version === EBIRD_SELECTED_VERSION ? buildEbirdSelectedResult : buildEbirdResult;
  ...
  if (sourceId === WQP_SOURCE) {
    ...
    const reconstruct = receipt.adapter_version === WQP_REVIEWED_FISH_VERSION ? buildWqpReviewedFishResult
      : receipt.adapter_version === WQP_FISH_VERSION ? buildWqpFishResult : buildWqpResult;
    const expected = reconstruct({runId, sourceId, stateCode, runStartedAt: receipt.started_at, parameters: receipt.parameters,
      requestedPairs: requestedPairKeys.map(key => { const [countyFips, speciesId] = key.split(":"); return {countyFips, speciesId, countyName: "Pinned registry", scientificName: speciesById.get(speciesId)!.scientificName}; })},
      p => readCommittedBytes(root, receipt.code_commit, p));
    for (const field of ["assertions", "reviews", "rejections", "outcomes", "upstreamRequests"] as const)
    ...
    if ((mentionsCoordinateDerivation && !explicitlyDeniesCoordinateDerivation) || includesCoordinateReceipts) {
      const coordinateContract = source.id === USFWS_EDNA_COORDINATE_SOURCE_ID
        ...
        : source.id === USFS_CURRENT_PLANTS_POLYGON_SOURCE_ID
          ...
          : source.id === USGS_BBS_ROUTE_START_SOURCE_ID
            ? {
                method: USGS_BBS_ROUTE_START_GEOGRAPHY_METHOD,
                policy: USGS_BBS_ROUTE_START_GEOGRAPHY_POLICY,
                topologyPath: USGS_BBS_ROUTE_START_TOPOLOGY_PATH,
              }
          : null;
      assert(
        coordinateContract &&
