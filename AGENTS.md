...
- Treat 25 percent available RAM as the preferred green target, not a universal dispatch prohibition. In green, start a newly calibrated desktop acquisition wave with four non-overlapping heavy lanes and scale toward six on the measured 32 GB desktop after a clean wave. A future host with at least 64 GB RAM and 200 GB free disk may test up to eight lanes.
- In yellow, run one or two acquisition workers, two to six lightweight workers, and one artifact-processing worker when their declared reservations preserve the 10 percent operational RAM reserve and the disk floors. Keep integration, compilation, and release gates sequential.
- In red, pause dispatched acquisition, artifact processing, compilers, builds, and deployments. Continue MAIN-owned low-memory source discovery, taxonomy or applicability review, receipt auditing, queue maintenance, and integration preparation. Stop only obsolete project-owned processes, never user applications, then remeasure and resume a safe tier automatically.
- Provider throttling can impose a lower ceiling than the machine. A provider-limited lane does not block independent jurisdictions, sources, reviews, or audits.
- Lightweight review may scale from eight toward sixteen lanes in green, and artifact processing from two toward four lanes, only when the corresponding resource tier and per-worker reservation pass.
