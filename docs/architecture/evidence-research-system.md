...
R2 publication is two phase. First upload missing content-addressed objects and the immutable release manifest, then verify remote bytes and public-domain samples. Only after those gates pass may MAIN promote the pointer, pin the release ID in the delivery target, deploy a preview, and release production. The publisher lists the complete bucket before every network write and refuses more than 8 GB retained storage, 800,000 monthly Class A requests, or 8,000,000 monthly Class B requests. Network modes require current-period Class A and Class B usage values from the Cloudflare billable-usage dashboard. These limits reserve 20 percent of every published R2 free-tier allowance and use decimal GB to match provider billing. Pointer promotions use a seven-day default minimum interval, with a specific override reason required for an urgent correction inside the interval. The read-only reachability report retains the current release plus two prior releases as its rollback window and separates current, rollback-only, historical-only, and unreferenced bytes. Neither the publisher nor the reporter deletes old releases automatically; deletion always requires separate explicit authority and fresh rollback verification.
...
## Independently agent-reviewed official state status (2026-09-08)

`official-jurisdiction-agent-review-v1` admits an explicit whole-state authority declaration through the existing jurisdiction evidence model. MAIN records the source interpretation and versioned parent; an independent source reviewer verifies the retained documents; a distinct leased evidence worker checks the exact county children and emits an immutable registered-adapter run. All three identities remain visible. This route records agent review and never manufactures human approval. Existing human-approved records retain their original approval receipts.

Version 1 supports complete active county sets for one explicitly reviewed state. It requires exact catalog taxonomy, retained source and witness hashes, a published explicit absence or eradication declaration, a separate current authority corroboration, and the exact independently reviewed dates and source URLs. An absent row, unsuccessful retrieval, quarantine removal alone, or survey silence supplies no negative evidence. Other geographic scope shapes require a separately tested method extension.

The declaration publication date, corroboration authority date, retrieval timestamps, and review expiry are separate. `authority-date-plus-365-days-v1` expires the project review 365 days after the dated corroboration; it does not guarantee biological absence for that period. The adapter cannot backdate assessment before its reviewed scope or use stale county projections. The California Karnal bunt pilot preserves the 2010-11-12 NAPPO declaration date, 2026-06-09 APHIS corroboration, and 2027-06-09 project review expiry. The retained 2013 USDA description of early-2011 administrative deregulation is a disclosed chronology discrepancy, not a redetection. No exact biological eradication date is claimed.

Dated earlier occurrence remains on the historical axis. Accepted later or undated occurrence blocks the affected child's current negative assertion. The pilot conservatively treats all occurrence dated 2010 or later as potentially conflicting. Each immutable run retains the reviewed parent and hashes of the exact county projections used for contradiction review. MAIN must recheck these baselines and any newly integrated evidence before accepting the worker run and compiling projections. A changed baseline requires a fresh contradiction evaluation; it cannot be silently treated as unchanged.

The runner pins all parent, independent review, authority artifacts, adapter inputs and county baselines to its code commit. MAIN alone integrates and compiles. Admission of a parent does not itself create county assertions, increase coverage, or complete research questions. Question-assessment methods retain their own source/version allowlists; this pilot does not claim question or protocol completion. Exact new determinations are measured from compiled unique pairs, separately from source outcomes and historical/current status.


### Agent-reviewed official status with year precision (precision-v2)

The separately versioned official-jurisdiction-agent-review-precision-v2 method uses the existing parent jurisdiction model and adapter official-jurisdiction-agent-reviewed at version 1.1.0. The existing v1 California method retains adapter 1.0.0 and its original declaration date. Existing human approvals and the question-assessment approval allowlist are unchanged.

An explicitly named-state authority status can retain an underlying information year while declaration publication and modification dates remain unknown. The parent source document stores informationYear; child source_record_date remains null. Never substitute January 1, retrieval, the global distribution-table modification date, or another authority's page date for the declaration date.

For v2 only, effectiveDateKind=corroboration-authority-date defines effectiveAt and the legacy reaffirmedAt field as operational metadata anchors from a separate current corroborating authority document. They do not establish declaration publication, biological onset, continuous absence, or demonstrated statement-specific reaffirmation. Temporal prose makes these distinctions explicit. Assessments cannot precede reviewedAsOf. The authority-date-plus-365-days-v1 expiry is a project review deadline, not an official guarantee. Refreshing retrieval alone does not renew it.

Admission requires explicit conditional support from a distinct independent reviewer, a matching supported statement type, no unresolved current conflict among the inspected source documents, and a matching presence-conflict policy. A structurally valid review with a dissenting disposition cannot authorize admission. Original interpretations, revised-method caveats, source documents, contextual correction documents, and independent reviews remain hashed and retained. Original proposals remain historical even when corrected method semantics supersede their wording.

Invalid-record correction is distinct from eradication. Precision-v2 officially-absent parents require presenceConflictPolicy=all-presence: any accepted genuine occurrence, including one older than the information year, blocks a child until specific adjudication. Correcting a misidentified series does not invalidate unrelated records. Eradication parents use from-conservative-boundary; later, overlapping or undated credible occurrence blocks current negative eligibility while older history remains visible. No correction deletes history. Unresolved statewide source conflicts also block admission; New Mexico's undated NPPO presence entry is held pending explicit adjudication.

Whole-state status supports exact active county membership through parent lineage; it does not establish independent county surveys. Current ledger contradiction checks are separate from source review and are repeated by MAIN at integration. No quarantine omission, missing row, failed retrieval, survey silence, or rejected taxonomy becomes absence. Method tests and source-review proposals create zero determinations. Actual gains require immutable worker runs, accepted reviews, integration and deterministic compilation.
