# Mississippi Research Progress

Generated: `2026-08-19T00:00:00.000Z`

## Exact Counts

- Species: `2504`
- Counties: `82`
- County-species pairs: `205328`
- Verified present: `3769`
- Verified absent: `0`
- Not detected: `0`
- Researched unresolved: `31491`
- Not researched: `170068`
- Determination coverage: `1.84%`
- Research coverage: `17.17%`
- Explicit outcome coverage: `17.1725%`
- Applicable protocol cells: `327`
- Current complete protocol cells: `108`
- Protocol completion: `37.00%`
- Current protocol completion: `33.03%`
- Evidence records: `6042`
- Research runs: `21`
- Rejection records: `608`
- Deferred source assertions remaining: `0`
- Deferred distinct pairs remaining: `0`
- Conflicts: `0`

Determination coverage counts only verified present and verified absent pairs. Research coverage also counts explicit not-detected evidence and source-family screens. Explicit outcome coverage counts completed immutable pair outcomes. None of these metrics implies absence.

## Highest Priority Species

| Species | Category | Not researched counties | Researched unresolved counties | Missing protocol sources |
| --- | --- | ---: | ---: | ---: |
| Emerald Ash Borer (`emerald-ash-borer`) | insects | 0 | 82 | 1 |
| Peach twig borer (`anarsia-lineatella`) | insects | 0 | 82 | 1 |
| Dogwood Anthracnose (`discula-destructiva`) | fungi-diseases | 0 | 82 | 1 |
| Asian Chestnut Gall Wasp (`dryocosmus-kuriphilus`) | insects | 0 | 82 | 1 |
| Hispid starbur (`acanthospermum-hispidum`) | plants | 0 | 82 | 0 |
| Devil's horsewhip (`achyranthes-aspera`) | plants | 0 | 82 | 0 |
| Sticky snakeroot (`ageratina-adenophora`) | plants | 0 | 82 | 0 |
| Tropical whiteweed (`ageratum-conyzoides`) | plants | 0 | 82 | 0 |
| Silver hairgrass (`aira-caryophyllea`) | plants | 0 | 82 | 0 |
| Chocolate Vine (`akebia-quinata`) | plants | 0 | 82 | 0 |
| Garlic Mustard (`alliaria-petiolata`) | plants | 0 | 82 | 0 |
| European Alder (`alnus-glutinosa`) | plants | 0 | 82 | 0 |
| Khakiweed (`alternanthera-pungens`) | plants | 0 | 82 | 0 |
| Purple amaranth (`amaranthus-blitum`) | plants | 0 | 82 | 0 |
| Largefruit amaranth (`amaranthus-deflexus`) | plants | 0 | 82 | 0 |
| African amaranth (`amaranthus-muricatus`) | plants | 0 | 82 | 0 |
| Mexican fruit fly (`anastrepha-ludens`) | insects | 0 | 82 | 0 |
| Asian Longhorned Beetle (`asian-longhorned-beetle`) | insects | 0 | 82 | 0 |
| Sweet Vernalgrass (`anthoxanthum-odoratum`) | plants | 0 | 82 | 0 |
| Wild chervil (`anthriscus-sylvestris`) | plants | 0 | 82 | 0 |
| Japanese angelica tree (`aralia-elata`) | plants | 0 | 82 | 0 |
| Common Burdock (`arctium-minus`) | plants | 0 | 82 | 0 |
| Capeweed (`arctotheca-calendula`) | plants | 0 | 82 | 0 |
| Hen's eyes (`ardisia-crenata`) | plants | 0 | 82 | 0 |
| Tall Oatgrass (`arrhenatherum-elatius`) | plants | 0 | 82 | 0 |

## Source Operations

| Source | Status | Last run | Evidence pairs | Screened species |
| --- | --- | --- | ---: | ---: |
| EDDMaps | operational | not run | 0 | 0 |
| USGS NAS | operational | 2026-07-16T09:29:02.000Z | 87 | 5 |
| USGS NAS occurrence API | operational | not run | 0 | 0 |
| SERNEC | operational | not run | 0 | 0 |
| EDDMapS ALIPC list | operational | not run | 0 | 0 |
| Alabama Plant Atlas | operational | not run | 0 | 0 |
| Alabama Forestry Commission Cogongrass GIS | operational | not run | 0 | 0 |
| Alabama Forestry Commission Aerial Detection layer | operational | not run | 0 | 0 |
| APHIS Emerald Ash Borer county layer | operational | not run | 0 | 0 |
| APHIS Federal Quarantine county layer | operational | 2026-08-15T03:54:52.560Z | 328 | 17 |
| APHIS National Honey Bee Survey | operational | not run | 0 | 0 |
| USFWS invasive carp eDNA sample layer | manual | not run | 0 | 0 |
| Laurel Wilt public county layer | operational | not run | 0 | 0 |
| USFS Alien Forest Pest Explorer | operational | 2026-07-26T03:59:30.000Z | 83 | 13 |
| USDA Forest Service FIA DataMart invasive plant tables | operational | 2026-07-26T13:44:30.000Z | 762 | 35 |
| USFS Current Invasive Plant Locations | operational | not run | 0 | 0 |
| USDA Forest Service Insect and Disease Survey point detections | operational | not run | 0 | 0 |
| USDA NRCS PLANTS county distribution | operational | 2026-08-17T05:41:05.189Z | 2445 | 316 |
| GBIF preserved specimen records | operational | 2026-08-19T08:54:58.386Z | 771 | 114 |
| GBIF coordinate-resolved preserved specimen records | operational | not run | 0 | 0 |
| GBIF Invasive Plant Atlas of the MidSouth records | operational | not run | 0 | 0 |
| GBIF eBird Observation Dataset | operational | not run | 0 | 0 |
| iDigBio preserved specimen records | operational | not run | 0 | 0 |
| iNaturalist Research Grade observations | operational | not run | 0 | 0 |
| Water Quality Portal biological results | operational | not run | 0 | 0 |
| USGS North American Breeding Bird Survey route-start detections | operational | not run | 0 | 0 |
| NEON TALL plant presence and percent cover | operational | not run | 0 | 0 |
| Manual authoritative evidence | manual | not run | 0 | 0 |
| Legacy merged presence lineage | legacy-migration | not run | 0 | 0 |
