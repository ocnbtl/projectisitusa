# New York Research Progress

Generated: `2026-08-03T00:00:00.000Z`

## Exact Counts

- Species: `2504`
- Counties: `62`
- County-species pairs: `155248`
- Verified present: `1032`
- Verified absent: `0`
- Not detected: `0`
- Researched unresolved: `3680`
- Not researched: `150536`
- Determination coverage: `0.66%`
- Research coverage: `3.04%`
- Explicit outcome coverage: `3.0351%`
- Applicable protocol cells: `97`
- Current complete protocol cells: `30`
- Protocol completion: `44.33%`
- Current protocol completion: `30.93%`
- Evidence records: `1036`
- Research runs: `8`
- Rejection records: `199`
- Deferred source assertions remaining: `0`
- Deferred distinct pairs remaining: `0`
- Conflicts: `0`

Determination coverage counts only verified present and verified absent pairs. Research coverage also counts explicit not-detected evidence and source-family screens. Explicit outcome coverage counts completed immutable pair outcomes. None of these metrics implies absence.

## Highest Priority Species

| Species | Category | Not researched counties | Researched unresolved counties | Missing protocol sources |
| --- | --- | ---: | ---: | ---: |
| Itchgrass (`rottboellia-cochinchinensis`) | plants | 62 | 0 | 1 |
| Wild sugarcane (`saccharum-spontaneum`) | plants | 62 | 0 | 1 |
| Giant Salvinia (`salvinia-molesta`) | plants | 62 | 0 | 1 |
| Scrambling nightshade (`solanum-tampicense`) | plants | 62 | 0 | 1 |
| Turkey berry (`solanum-torvum`) | plants | 62 | 0 | 1 |
| Tropical soda apple (`solanum-viarum`) | plants | 62 | 0 | 1 |
| Simple-stem burr-reed (`sparganium-erectum`) | plants | 62 | 0 | 1 |
| Coatbuttons (`tridax-procumbens`) | plants | 62 | 0 | 1 |
| Panic liverseed grass (`urochloa-panicoides`) | plants | 62 | 0 | 1 |
| Camphor scale (`aspidiotus-duplex`) | insects | 0 | 62 | 2 |
| Phytophthora Root Rot (`phytophthora-cinnamomi`) | fungi-diseases | 0 | 62 | 2 |
| Laurel wilt (`raffaelea-lauricola`) | fungi-diseases | 0 | 62 | 2 |
| Punktree (`melaleuca-quinquenervia`) | plants | 0 | 62 | 1 |
| Sticky snakeroot (`ageratina-adenophora`) | plants | 0 | 62 | 0 |
| Mimosa Tree (`albizia-julibrissin`) | plants | 0 | 62 | 0 |
| Sessile joyweed (`alternanthera-sessilis`) | plants | 0 | 62 | 0 |
| Capeweed (`arctotheca-calendula`) | plants | 0 | 62 | 0 |
| Onionweed (`asphodelus-fistulosus`) | plants | 0 | 62 | 0 |
| Animated oat (`avena-sterilis`) | plants | 0 | 62 | 0 |
| Feathered mosquitofern (`azolla-pinnata`) | plants | 0 | 62 | 0 |
| Jio (`commelina-benghalensis`) | plants | 0 | 62 | 0 |
| Common crupina (`crupina-vulgaris`) | plants | 0 | 62 | 0 |
| Anchored water hyacinth (`eichhornia-azurea`) | plants | 0 | 62 | 0 |
| Russian Olive (`elaeagnus-angustifolia`) | plants | 0 | 62 | 0 |
| Southern threecornerjack (`emex-australis`) | plants | 0 | 62 | 0 |

## Source Operations

| Source | Status | Last run | Evidence pairs | Screened species |
| --- | --- | --- | ---: | ---: |
| EDDMaps | operational | not run | 0 | 0 |
| USGS NAS | operational | 2026-07-16T09:29:03.000Z | 154 | 5 |
| USGS NAS occurrence API | operational | not run | 0 | 0 |
| SERNEC | operational | not run | 0 | 0 |
| EDDMapS ALIPC list | operational | not run | 0 | 0 |
| Alabama Plant Atlas | operational | not run | 0 | 0 |
| Alabama Forestry Commission Cogongrass GIS | operational | not run | 0 | 0 |
| Alabama Forestry Commission Aerial Detection layer | operational | not run | 0 | 0 |
| APHIS Emerald Ash Borer county layer | operational | not run | 0 | 0 |
| APHIS Federal Quarantine county layer | operational | not run | 0 | 0 |
| APHIS National Honey Bee Survey | operational | not run | 0 | 0 |
| USFWS invasive carp eDNA sample layer | manual | not run | 0 | 0 |
| Laurel Wilt public county layer | operational | not run | 0 | 0 |
| USFS Alien Forest Pest Explorer | operational | 2026-07-26T03:59:30.000Z | 374 | 13 |
| USDA Forest Service FIA DataMart invasive plant tables | operational | 2026-07-26T13:44:30.000Z | 494 | 33 |
| USFS Current Invasive Plant Locations | operational | not run | 0 | 0 |
| USDA Forest Service Insect and Disease Survey point detections | operational | not run | 0 | 0 |
| USDA NRCS PLANTS county distribution | operational | not run | 0 | 0 |
| GBIF preserved specimen records | operational | 2026-08-02T21:08:53.475Z | 10 | 25 |
| GBIF coordinate-resolved preserved specimen records | operational | not run | 0 | 0 |
| GBIF Invasive Plant Atlas of the MidSouth records | operational | not run | 0 | 0 |
| GBIF eBird Observation Dataset | operational | not run | 0 | 0 |
| iDigBio preserved specimen records | operational | not run | 0 | 0 |
| iNaturalist Research Grade observations | operational | not run | 0 | 0 |
| Water Quality Portal biological results | operational | not run | 0 | 0 |
| USGS North American Breeding Bird Survey route-start detections | operational | not run | 0 | 0 |
| NEON TALL plant presence and percent cover | operational | not run | 0 | 0 |
| Manual authoritative evidence | manual | not run | 0 | 0 |
| Legacy merged presence lineage | legacy-migration | not run | 0 | 0 |
