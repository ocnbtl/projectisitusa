# Utah Research Progress

Generated: `2026-09-02T00:00:00.000Z`

## Exact Counts

- Species: `2504`
- Counties: `29`
- County-species pairs: `72616`
- Verified present: `2332`
- Verified absent: `29`
- Not detected: `0`
- Researched unresolved: `11762`
- Not researched: `58493`
- Determination coverage: `3.25%`
- Research coverage: `19.45%`
- Explicit outcome coverage: `19.4489%`
- Applicable protocol cells: `294`
- Current complete protocol cells: `70`
- Protocol completion: `28.23%`
- Current protocol completion: `23.81%`
- Evidence records: `2401`
- Research runs: `31`
- Rejection records: `72`
- Deferred source assertions remaining: `0`
- Deferred distinct pairs remaining: `0`
- Conflicts: `0`

Determination coverage counts only verified present and verified absent pairs. Research coverage also counts explicit not-detected evidence and source-family screens. Explicit outcome coverage counts completed immutable pair outcomes. None of these metrics implies absence.

## Highest Priority Species

| Species | Category | Not researched counties | Researched unresolved counties | Missing protocol sources |
| --- | --- | ---: | ---: | ---: |
| Emerald Ash Borer (`emerald-ash-borer`) | insects | 0 | 29 | 2 |
| Camphor scale (`aspidiotus-duplex`) | insects | 0 | 29 | 2 |
| Chestnut Blight (`cryphonectria-parasitica`) | fungi-diseases | 0 | 29 | 2 |
| Dogwood Anthracnose (`discula-destructiva`) | fungi-diseases | 0 | 29 | 2 |
| Asian Chestnut Gall Wasp (`dryocosmus-kuriphilus`) | insects | 0 | 29 | 2 |
| Mimosa webworm (`homadaula-albizziae`) | insects | 0 | 29 | 2 |
| Cottony cushion scale (`icerya-purchasi`) | insects | 0 | 29 | 2 |
| Butternut Canker Fungus (`ophiognomonia-clavigignenti-juglandacearum`) | fungi-diseases | 0 | 29 | 2 |
| Japanese Beetle (`popillia-japonica`) | insects | 0 | 29 | 2 |
| Laurel wilt (`raffaelea-lauricola`) | fungi-diseases | 0 | 29 | 2 |
| Common crupina (`crupina-vulgaris`) | plants | 0 | 29 | 1 |
| Giant Hogweed (`giant-hogweed`) | plants | 0 | 29 | 1 |
| Hydrilla (`hydrilla`) | plants | 0 | 29 | 1 |
| Cogongrass (`imperata-cylindrica`) | plants | 0 | 29 | 1 |
| Florida keys indian mallow (`abutilon-hirtum`) | plants | 0 | 29 | 0 |
| Cedar wattle (`acacia-elata`) | plants | 0 | 29 | 0 |
| Pearl wattle (`acacia-podalyriifolia`) | plants | 0 | 29 | 0 |
| New Zealand burr (`acaena-magellanica`) | plants | 0 | 29 | 0 |
| Yellowfin goby (`acanthogobius-flavimanus`) | wildlife | 0 | 29 | 0 |
| Mysid (`acanthomysis-aspera`) | wildlife | 0 | 29 | 0 |
| Paraguayan starbur (`acanthospermum-australe`) | plants | 0 | 29 | 0 |
| Hispid starbur (`acanthospermum-hispidum`) | plants | 0 | 29 | 0 |
| Honey bee tracheal mite (`acarapis-woodi`) | wildlife | 0 | 29 | 0 |
| Goji gall mite (`aceria-kuko`) | wildlife | 0 | 29 | 0 |
| Litchi mite (`aceria-litchii`) | wildlife | 0 | 29 | 0 |

## Source Operations

| Source | Status | Last run | Evidence pairs | Screened species |
| --- | --- | --- | ---: | ---: |
| EDDMaps | operational | 2026-09-02T08:47:39.490Z | 813 | 138 |
| USGS NAS | operational | 2026-07-16T09:29:04.000Z | 26 | 5 |
| USGS NAS occurrence API | operational | not run | 0 | 0 |
| SERNEC | operational | not run | 0 | 0 |
| EDDMapS ALIPC list | operational | not run | 0 | 0 |
| Alabama Plant Atlas | operational | not run | 0 | 0 |
| Alabama Forestry Commission Cogongrass GIS | operational | not run | 0 | 0 |
| Alabama Forestry Commission Aerial Detection layer | operational | not run | 0 | 0 |
| APHIS Emerald Ash Borer county layer | operational | not run | 0 | 0 |
| APHIS Federal Quarantine county layer | operational | 2026-08-15T03:54:52.560Z | 0 | 17 |
| APHIS National Honey Bee Survey | operational | not run | 0 | 0 |
| USFWS invasive carp eDNA sample layer | operational | not run | 0 | 0 |
| Laurel Wilt public county layer | operational | not run | 0 | 0 |
| USFS Alien Forest Pest Explorer | operational | 2026-07-26T03:59:30.000Z | 20 | 13 |
| USDA Forest Service FIA DataMart invasive plant tables | operational | 2026-07-26T13:44:30.000Z | 75 | 67 |
| USFS Current Invasive Plant Locations | operational | not run | 0 | 0 |
| USDA Forest Service Insect and Disease Survey point detections | operational | not run | 0 | 0 |
| USDA NRCS PLANTS county distribution | operational | 2026-08-17T05:41:05.189Z | 1375 | 316 |
| GBIF preserved specimen records | operational | 2026-08-21T00:14:35.196Z | 36 | 92 |
| GBIF coordinate-resolved preserved specimen records | operational | not run | 0 | 0 |
| GBIF Invasive Plant Atlas of the MidSouth records | operational | not run | 0 | 0 |
| GBIF eBird Observation Dataset | operational | not run | 0 | 0 |
| iDigBio preserved specimen records | operational | not run | 0 | 0 |
| iNaturalist Research Grade observations | operational | not run | 0 | 0 |
| Water Quality Portal biological results | operational | not run | 0 | 0 |
| USGS North American Breeding Bird Survey route-start detections | operational | not run | 0 | 0 |
| NEON TALL plant presence and percent cover | operational | not run | 0 | 0 |
| USDA APHIS northern giant hornet eradication statement (2024) | manual | 2026-09-01T19:16:53.000Z | 29 | 1 |
| WSDA northern giant hornet eradication statement (2024) | manual | not run | 0 | 0 |
| WSDA 2025 northern giant hornet surveillance update | manual | not run | 0 | 0 |
| NJDEP Asian longhorned beetle eradication status | manual | not run | 0 | 0 |
| USDA APHIS Asian longhorned beetle program update (2026) | manual | not run | 0 | 0 |
| Manual authoritative evidence | manual | not run | 0 | 0 |
| Legacy merged presence lineage | legacy-migration | not run | 0 | 0 |
