...
`npm run research:publication:manifest` builds a deterministic content-addressed release manifest for `public/generated/research`. New releases can use HTTP gzip with a fresh inventory, as described in [compressed publication](compressed-publication.md). Existing raw or compressed objects retain their exact stored representation. `npm run research:publication:plan` reports object, byte, and free-tier safety budgets without credentials or provider writes. Supply freshly observed, conservatively rounded-up Cloudflare billing-period Class A and Class B counters to network modes; zero is valid only when independently observed. The publisher lists the complete bucket, uploads only missing hashes, and verifies the immutable release. It fails above 8 GB retained storage, 800,000 monthly Class A requests, or 8,000,000 monthly Class B requests. Promotion requires `--mode publish --verification full --public-origin https://data.isitusa.com --app-origin https://isitusa.com --promote`; both public delivery paths are checked before the pointer changes. Research-data pointer promotions use a seven-day minimum interval so validated research can accumulate locally; an urgent correctness release inside that window must include a specific, single-line `--cadence-override-reason` for the production receipt.

`npm run research:r2:reachability` is a read-only R2 inventory command. On Windows, run `scripts/research/run-r2-reachability-report.ps1` with a freshly observed Cloudflare Class A count, Class B count, and dashboard timestamp. The report uses only bucket LIST and GET requests, reconciles `current.json` with every immutable release manifest, and reports exact current, rollback-only, historical-only, and unreferenced object bytes. The rollback reporting window is the current release plus the two most recent prior releases. The command never uploads, promotes, or deletes; every cleanup candidate remains informational until separate explicit deletion authority and a fresh rollback verification exist.

On the canonical Windows host, `scripts/research/run-r2-publisher.ps1` loads the bucket-scoped access key from `%LOCALAPPDATA%\ProjectIsitusa\credentials\r2-publisher.clixml`. The credential file is protected for the current Windows user with DPAPI and remains outside the repository. Network modes still require explicit current-period operation counts, so a retained credential cannot bypass the free-tier preflight. Do not print, copy into logs, or commit either R2 credential value.

The current delivery configuration uses `r2` mode and resolves `https://data.isitusa.com/current.json` to an immutable release manifest. The client verifies decoded JSON bytes and SHA-256 before parsing. Transparent HTTP gzip preserves that reader contract. Retain the previous pointer and immutable manifests for rollback; the configured GitHub route also supports an exact source-commit fallback. Verify the live pointer and app deployment separately because research-data promotion and app deployment have different identities. Before releasing app data, run `npm run check:release-coherence`; the [app/research coherence gate](app-research-release-coherence.md) holds hosted builds whose authoritative county presence differs from the live research release.
...
## EPA NRSA 2018/2019 retained review

EPA adapter version `2.0.0` adds a separate, hash-pinned 2018/2019 method. The fish table uses different UID values from the sampling and site tables. A unique site/date/visit/state join links them, and original identifiers remain unchanged. Historical and later taxon lists must agree, with the retained 2018 field manual supplying exact scientific/common-name corroboration for three later-reference-only taxa. A positive recorded catch can remain usable despite insufficient assessment effort; sampling exclusions and geographic contradictions still receive explicit review.

The September8 method supports293 candidate pairs and holds five. These are method decisions until immutable runs and national reconciliation establish actual progress. `npm run check:epa-nrsa-1819-fish` verifies exact bytes, all40 states, event schemas, attribution, original proposal links and76 adverse method cases. The earlier adapter remains version1. A separate scoped review records the introduced walking-catfish concept issue for prior assertions; no absence or automatic retraction follows from that concern.

## EPA NRSA 2013/2014 retained review

EPA adapter version `3.0.0` adds a separate 2013/2014 method without changing earlier adapter contracts. The original fish UID and unique complete site/date/visit/postal-state join must identify the same site. Contemporaneous TAXA_ID, common name and scientific name must agree. Positive integer counts and sampled-site flags are required; repeat visits and anomaly counts remain source context.

Four-digit numeric publisher FIPS can restore one leading zero only with independently matching active county name and primary postal state. Explicit two-state river qualifiers must equal BORD_RIV and contain that state. The original Baltimore name remains unchanged; its publisher FIPS disambiguates county from independent city. Small generalized-boundary offsets alone do not override exact publisher identity. Only the hash-pinned site file uses reversible single-byte decoding for its two original 0xF1 bytes; source bytes and row terminators remain unchanged.

The September 8 method supports 230 candidate pairs across 40 states from 282 qualifying observations and retains one contradictory additional row as a grouped rejection. Two Florida Pterygoplichthys disjunctivus pairs remain held for exact identification provenance; regional hybrid context does not reidentify the EPA fish. These method counts are potential gains until immutable runs and national reconciliation prove net unique movement. `npm run check:epa-nrsa-1314-fish` verifies source bytes, all state fixtures, compiler attribution, original worker-commit lineage, county and taxon identity, dates and adverse mutations. Missing or rejected records never establish absence or non-detection.
