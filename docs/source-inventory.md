...
| --- | --- | --- | --- | --- |
| Official Alabama, Alaska, Arkansas, and Arizona invasive or regulated species lists | State-level regulated applicability only, separate from county evidence and source-screen completion | `src/data/research/state-applicability-source-registry.json`, `src/data/research/state-list-sources/`, `src/data/research/state-applicability/AL.json`, `src/data/research/state-applicability/AK.json`, `src/data/research/state-applicability/AR.json`, `src/data/research/state-applicability/AZ.json` | Official Alabama and Alaska PDFs and the Arkansas rule HTML were retained with exact hashes and bytes. The official Arizona Secretary of State Title 3 Chapter 4 PDF is retained at `2,738,828` bytes with SHA-256 `b49c4428f1182fbe458de2d6219c4694aa556de617d212db2fec18c1a4347308`; its R3-4-245 review covers all `60` rows in Tables 4-6 with `40` accepted events across `39` species and `20` blocked rows. The current Title 12 Chapter 4 PDF is retained at `5,842,257` bytes with SHA-256 `416d8c1ef00b21128bcab7c53efcf5eb4526c0d9170ffa2a34efaf1622a0bb8a`; the R12-4-406 review covers all `53` rows in subsections C and J-L with `8` exact-binomial events and `45` blocked rows. `npm run research:apply:state-lists` validates descriptors, permits only exact current-catalog binomials, and merges reviewed bases deterministically. List membership establishes state regulatory applicability only and never county occurrence, absence, non-detection, or not-applicability. | Alabama, Alaska, and Arkansas retrieved and reviewed on `2026-07-28`; Arizona Title 3 reviewed on `2026-08-04` from Supplement 25-1 and Title 12 reviewed on `2026-08-05` from Supplement 26-2 dated `2026-06-30` |
...
| EDDMapS, Early Detection and Distribution Mapping System, https://www.eddmaps.org/ | County-level invasive species presence for exact catalog matches and curated subject IDs in the merged county snapshot | `src/data/source/eddmaps-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-eddmaps.ts` refreshes the public EDDMapS subject summary and county presence endpoints, writing exact catalog and curated subject matches to `src/data/source/eddmaps-snapshot.json`. `scripts/merge-eddmaps-county-presence.ts` provides a bounded merge path for refreshing EDDMapS rows in the county snapshot when unrelated live source families are too slow. | Refreshed and merged on `2026-04-26`: `1409` EDDMapS species, `346716` raw county rows, `1402` lower-48 merged source species |
...
| USGS NAS occurrence API, https://nas.er.usgs.gov/api/v2/occurrence/search | Targeted county-level occurrence support for reviewed Alabama ANS denominator gaps not covered by the local NAS archive import | `src/data/source/nas-alabama-api-supplements.ts`, `src/data/source/usgs-nas-alabama-api-supplement-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:nas-alabama-api` queries eight reviewed exact NAS species IDs through the structured API, accepts explicit Alabama county rows, skips failed and no-county rows, restricts Blue Tilapia to established rows, and excludes the reviewed Panama City Beach locality incorrectly assigned to Houston County, Alabama. `scripts/import-county-presence.ts` uses the same target registry during full rebuilds. The July 11 pass accepted `166` records across `38` Alabama county-species source pairs and added `14` pairs beyond existing source coverage. Treat accepted rows as occurrence or collection evidence only. | Accessed and merged on `2026-07-11` |
...
| Alabama Plant Atlas, http://floraofalabama.org/ | County-level Alabama plant occurrence support from vouchered Alabama Herbarium Consortium distribution maps | `src/data/source/state-species-denominators.ts`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-county-presence.ts` queries the public Alabama Plant Atlas scientific-name API for exact catalog plant matches, then imports Alabama county FIPS distribution rows from the public API. Reviewed accepted-name aliases are used only where recorded, such as `Camphora officinarum` for `Cinnamomum camphora` and `Dioscorea polystachya` for `Dioscorea oppositifolia` | Accessed and merged on `2026-04-25` |
| Invasive Plant Atlas of the MidSouth via GBIF, https://www.gbif.org/dataset/d587c7e5-d442-437a-a6d7-d1a78ecf2300 | Alabama county-level invasive plant occurrence support for exact current-catalog plant taxa | `src/data/source/gbif-ipams-alabama-plants-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:gbif-ipams-alabama` queries the IPAMS occurrence dataset through GBIF, bounds records to an Alabama bounding box, requires `HUMAN_OBSERVATION`, `PRESENT`, coordinates, no GBIF geospatial issue flag, local point-to-county resolution against Alabama county geometry, and accepted GBIF scientific names that start with an exact current-catalog plant binomial. The 2026-06-26 pass scanned `2701` bounded records, resolved `1727` inside Alabama counties, accepted `1641` records, imported `61` species, produced `308` gross Alabama county-species pairs, and added `6` net Alabama matrix determinations after generated unioning. Treat IPAMS rows as invasive-plant observed-presence evidence, not countywide absence, survey non-detection, establishment proof, abundance, or complete county plant inventory coverage. | Accessed and merged on `2026-06-26` |
| USDA Forest Service EDW Current Invasive Plant Locations, https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_InvasiveSpecies_01/MapServer/0 | Current Alabama National Forest invasive plant infestation polygon support for exact catalog species; national occurrence-only targeted research | `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json`, `ops/national-research/evaluations/usfs-current-invasive-plants-national-preflight-20260827-r1.json`, `ops/national-research/evaluations/usfs-current-invasive-plants-or-pilot-acquisition-20260827-r1.json`, `src/data/research/national-acquisition-plans/usfs-current-invasive-plants-or-pilot-v1.json`, `src/data/research/runs/20260827T234500Z__usfs-current-invasive-plants__ae4b9e8ce5c2/` | The legacy path in `scripts/import-county-presence.ts` filters to `NFS IN ALABAMA NATIONAL FOREST`, matches exact accepted scientific names, and uses polygon bbox centers only for the historical Alabama snapshot. `scripts/research/build-usfs-current-invasive-plants-preflight.ts` performs a bounded national GET-only capability, taxonomy, date-quality, and overlap preflight. Its bbox-center sample selects candidates only and cannot publish geography. The 2026-08-27 preflight observed `946008` current rows, `517` exact catalog taxa covering `785457` rows, `2439` rows without an accepted name, `189` pre-1900 or future collected dates, and `30` not-researched pairs in a `157`-pair resolved sample. `scripts/research/adapters/usfs-current-invasive-plants-targeted.ts` may publish a selected positive record only after two normalized-identical retained responses, exact taxonomy, valid nonfuture dates, unique object identity, and at least one retained full-polygon vertex inside the committed active county topology. The Oregon pilot retained two normalized-identical 17-record responses, accepted 15 of 16 requested pairs as recorded presence, and classified the remaining geography-ambiguous selection as no qualifying evidence. Failed selections and source silence never establish absence or non-detection. The official clearinghouse also advertises a dated `338` MB national File Geodatabase for a later complete-national path. | Alabama snapshot accessed and merged on `2026-04-26`; national preflight and Oregon pilot observed on `2026-08-27`; the successful Oregon run issued exactly two provider GETs, no provider POSTs, and no negative claims |
| USDA Forest Service FIA DataMart invasive plant tables, https://apps.fs.usda.gov/fia/datamart/datamart.html | Plot-observed Alabama invasive plant county support for exact catalog species | `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:fia-alabama-invasive-plants` downloads `AL_INVASIVE_SUBPLOT_SPP.csv`, `REF_PLANT_DICTIONARY.csv`, and `REF_INVASIVE_SPECIES.csv`, requires Alabama invasive-reference symbols plus exact current catalog scientific-name matches, normalizes FIA `COUNTYCD` to Alabama county FIPS, and imports verified-present county rows only. `scripts/import-county-presence.ts` includes the same source family for full county refreshes. Genus-level or non-exact symbols including `BAMBU`, `DIOSC`, `ELAEA`, `HEDER`, `LESPE`, `LIGUS2`, `LIRIO2`, `LONIC`, `MABE2`, `POTR4`, `ROSA5`, `VINCA`, and `WISTE` are skipped. Treat FIA rows as plot observation presence evidence, not countywide absence or complete county inventory coverage. | Accessed and merged on `2026-06-15` |
| USDA APHIS Federal Quarantine FeatureServer, https://services7.arcgis.com/2C1NQ7u6M6SXoa8p/arcgis/rest/services/PPQ_GIS_Federal_Quarantine_AGOL_EDIT_Feature_Layer_view/FeatureServer/1 | National regulatory recorded-present evidence for 17 exact reviewed quarantine-program mappings across explicit active county FIPS | `src/data/research/national-acquisition-plans/aphis-federal-quarantine-national-v1.json`, `src/data/research/national-acquisitions/*__aphis-federal-quarantine__*/`, `src/data/research/runs/*__aphis-federal-quarantine__*/` | `npm run research:acquire:aphis-quarantine-national` captures the layer once with a stable edit-window check, retains all raw JSON responses, and partitions locally into deterministic state runs. Publication requires an accepted current regulatory status, exact reviewed program mapping, explicit active FIPS, matching state, and matching county name. Coordinates and automatic successor inference are prohibited. Regulatory evidence supports the named program condition only. Pending-only, rescinded, rejected, unmapped, or silent rows never establish absence or non-detection. | National actor registered `2026-08-15`; each immutable receipt records its exact retrieval time and layer edit timestamp |
| USDA NRCS PLANTS county distribution, https://apps.geo.fpac.usda.gov/nrcs-geodata/rest/services/land_use_land_cover/plants/MapServer/6 | Alabama county-level PLANTS distribution and nativity rows for exact current-catalog plant taxa | `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:nrcs-plants-alabama` queries the official PLANTS state layer for Alabama `Introduced` taxa, resolves each accepted PLANTS profile by `plant_master_id`, requires exact current catalog taxon-name matches, queries county distribution geometries, spatially resolves returned county polygons to local Alabama county FIPS, and imports only county rows with `Introduced` or `Both` nativity symbols. County-layer internal ids are not treated as FIPS. Treat PLANTS rows as county distribution and nativity evidence, not specimen, abundance, survey non-detection, countywide absence, or invasive impact evidence. | Accessed and merged on `2026-06-15` |
| NEON TALL plant presence and percent cover, https://data.neonscience.org/data-products/DP1.10058.001 | Plot-level Alabama plant presence and cover support for exact current-catalog introduced plant taxa at the Talladega National Forest NEON site | `src/data/source/neon-tall-alabama-plants-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:neon-tall-alabama` queries official NEON `DP1.10058.001` metadata for `TALL` `RELEASE-2026` months, downloads expanded `div_1m2Data` CSVs, requires `divDataType=plantSpecies`, species rank, blank `identificationQualifier`, `nativeStatusCode=I`, exact current catalog binomial matches, and row coordinates resolving to exactly one Alabama county. The pass reviewed `24` released months and `30239` rows, accepted `48` records for `9` exact catalog species, produced `13` gross county-species source pairs, and added `6` net Alabama matrix determinations. Treat NEON rows as plot observation presence and cover evidence, not countywide absence, sampled-plot non-detection, establishment proof, invasive-impact proof, or complete county plant inventory coverage. | Accessed and merged on `2026-06-16` |
...
| Alabama Forestry Commission Aerial Detection FeatureServer, https://gis.forestry.alabama.gov/arcgis/rest/services/AFCEnterprise/AerialDetectionP/FeatureServer/0 | County-level Alabama aerial detection support for exact catalog matches in the merged county snapshot | `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:afc-aerial-detections` and `scripts/import-county-presence.ts` query the public ArcGIS FeatureServer layer for reviewed `TypeDetected` codes, import only exact catalog mappings for `EAB` to `Agrilus planipennis`, `Cogongrass` to `Imperata cylindrica`, and `Laurel Wilt Disease` to `Raffaelea lauricola`, and skip `SPB`, `IPS`, `Needle Blight`, `Sawfly`, abiotic rows, and infrastructure rows without reviewed exact catalog species targets. Treat this as detection presence evidence only, not absence or complete statewide inventory coverage. | Accessed and merged on `2026-06-14` |
| USDA Forest Service Insect and Disease Survey IDS POINTS layer, https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_InsectandDiseaseSurvey_01/MapServer/0 | County-level Alabama forest insect and disease point detections for reviewed exact catalog matches | `src/data/source/usfs-ids-alabama-point-detections-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:usfs-ids-alabama` queries the official EDW IDS POINTS layer for reviewed exact DCA labels, requires exact current catalog scientific-name mappings, matching DCA codes, point geometry, and coordinates resolving to exactly one Alabama county. The pass reviewed `3` exact target labels, accepted `11` Alabama point detections for `3` species, produced `7` gross county-species source pairs, and added `2` net Alabama matrix determinations. `fire`, `drought`, `salt damage`, `wind-tornado/hurricane`, `unknown`, `unknown bark beetle`, `ips engraver beetles`, `southern pine beetle`, and `black turpentine beetle` remain skipped because they are abiotic, unknown, broad, native, or not exact current catalog invasive-species mappings. Treat IDS points as forest-health detection presence evidence only, not host-confirmed occurrence, absence, non-detection, establishment proof, or complete county inventory coverage. | Accessed and merged on `2026-06-16` |
...
| USDA APHIS PPQ Federal Quarantine county FeatureServer, https://services7.arcgis.com/2C1NQ7u6M6SXoa8p/arcgis/rest/services/PPQ_GIS_Federal_Quarantine_AGOL_EDIT_Feature_Layer_view/FeatureServer/1 | County-level Alabama regulatory quarantine support for exact catalog-matched pest and pathogen programs | `src/data/source/aphis-hlb-alabama-quarantine-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-county-presence.ts` queries active Alabama quarantine rows, maps reviewed program names to exact catalog species, and imports Asian citrus psyllid, red imported fire ant, sweet orange scab, and citrus greening HLB county coverage. `npm run import:aphis-hlb-alabama` separately rechecked the active Alabama HLB rows after the catalog gained `Liberibacter asiaticus`, accepting Baldwin and Mobile counties for `Citrus Greening (HLB)`. Treat APHIS quarantine rows as regulatory county evidence, not specimen, lab-result, absence, non-detection, or complete survey evidence. | Accessed and merged on `2026-06-16` |
...
| Alien Forest Pest Detection by Counties in the United States, Purdue University Research Repository, DOI: https://doi.org/10.4231/HWQF-V087 | County-level forest pest and pathogen detections from the USFS Alien Forest Pest Explorer source family | `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-county-presence.ts` downloads the PURR bundle, parses `AFPE_PEST_2023_counties.csv`, and imports reviewed AFPE pest-code matches that resolve to catalog species | Accessed and merged on `2026-04-25` |
| Alien Forest Pest Detection by Counties in the United States v1.0 immutable research acquisition, Purdue University Research Repository, DOI: https://doi.org/10.4231/HWQF-V087 | Historical county-level forest pest and pathogen detection evidence plus completed no-qualifying-evidence source screens for `13` reviewed DCA mappings across the national v1 county-equivalent registry | `src/data/research/national-acquisitions/20240328__usfs-afpe-v1-0__ca1988d2f900`, `src/data/research/runs/*__usfs-afpe__*`, `src/data/research/source-mappings/usfs-afpe-v1.json`, `ops/national-research/evaluations/usfs-afpe-national-2026-07-26.json` | `npm run research:acquire:usfs-afpe-national` retains the `CC0-1.0` archive once. `npm run research:partition:usfs-afpe-national` uses exact current FIPS and reviewed mappings to partition it locally across `51` jurisdictions. A value of `1` supports recorded-present evidence. A value of `0` supports only no-qualifying-evidence research completion and never absence or not detected. Missing or retired geography remains blocked or rejected. The v1.0 data is stale, with source data last updated in April 2023. PURR metadata describes `89` pests while the CSV and dictionary contain `93` DCA columns. The live v2 application is newer and volatile and was not substituted for the hash-pinned archive. | Acquired and partitioned on `2026-07-26`; archive SHA-256 `ca1988d2f900a71ff5efab2b32642f757d64189c4bd0dd2dbdda98a6cf222383` |
| GBIF occurrence search, https://www.gbif.org/occurrence/search | County-level Alabama physical occurrence support from exact catalog scientific-name matches with preserved specimen records | `src/data/source/gbif-alabama-preserved-specimens-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-gbif-alabama-specimens.ts` matches generated catalog species against the GBIF backbone with strict exact species matches, imports Alabama `PRESERVED_SPECIMEN` records with explicit county values, filters cultivated-looking contexts, and `scripts/import-county-presence.ts` merges the resulting snapshot. Treat this as verified physical occurrence evidence, not proof of established invasive population or absence. | Accessed and merged on `2026-04-25` |
| GBIF coordinate-resolved preserved specimen records, https://www.gbif.org/occurrence/search | Targeted Alabama physical occurrence support where GBIF preserved specimens have coordinates but not necessarily explicit county text | `src/data/source/gbif-alabama-coordinate-specimens-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:gbif-alabama-coordinate-specimens` queries a bounded exact-species supplement identified from GBIF Alabama specimen facets. It requires `PRESERVED_SPECIMEN`, `PRESENT`, coordinates, no GBIF geospatial issue flag, an Alabama bounding-box hit, local point-to-county resolution against Alabama county geometry, exact current-catalog species, and exact GBIF species keys. The importer excludes cultivated, planted, garden, nursery, arboretum, landscaped, and lab-colony context text. The expanded supplement now reviews `68` exact targets, imports `66` species, accepts `3572` specimen records, produces `904` gross Alabama county-species source pairs, and added `24` net Alabama matrix determinations in the latest pass after generated unioning. The newest `45` target expansion accepted county evidence for `44` species, while `Xylosandrus crassiusculus` had GBIF records but no accepted county-resolved preserved specimens under the current filters. Treat this as physical specimen occurrence evidence, not establishment proof, invasive-impact proof, abundance, absence, non-detection, or complete county inventory coverage. | Accessed and merged on `2026-07-06` |
| GBIF eBird Observation Dataset, https://www.gbif.org/dataset/4fa7b334-ce0d-4e88-aaae-2e0c138d049e | County-level Alabama observed-presence support for exact current-catalog bird taxa | `src/data/source/gbif-ebird-alabama-birds-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:gbif-ebird-alabama` queries the public GBIF EOD dataset for a bounded exact bird pilot, requires exact GBIF species matches, `country=US`, `stateProvince=Alabama`, `basisOfRecord=HUMAN_OBSERVATION`, `occurrenceStatus=PRESENT`, coordinates without GBIF geospatial issues, and county resolution by local Alabama county geometry or unambiguous county names. The source audit stores one representative GBIF occurrence per imported county, total GBIF occurrence counts, scanned counts, and accepted counts. The expanded pilot reviewed `10` exact current-catalog bird species, imported `7` species, scanned and accepted `31696` occurrence records, produced `292` gross county-species source pairs, and added `21` net Alabama matrix determinations in the latest pass. `Aratinga erythrogenys`, `Brotogeris versicolurus`, and `Pycnonotus jocosus` had exact catalog and GBIF species matches but no Alabama EOD rows, so they remain unknown from this source. Treat GBIF EOD as annual eBird occurrence-only observed-presence evidence, not checklist non-detection, agency survey evidence, establishment proof, invasive-impact proof, or countywide absence. | Accessed and merged on `2026-06-28` |
| USGS North American Breeding Bird Survey, DOI: https://doi.org/10.5066/P144YU3S | Official route-start survey-detection support for exact current-catalog bird taxa | `src/data/source/bbs-alabama-route-start-birds-snapshot.json`, `src/data/research/national-acquisition-plans/usgs-bbs-tx-route-start-pilot-v1.json`, `src/data/research/national-acquisition-plans/usgs-bbs-il-route-start-tranche-v1.json`, `src/data/research/national-acquisition-plans/usgs-bbs-in-route-start-tranche-v1.json`, `src/data/research/national-acquisition-plans/usgs-bbs-ca-route-start-tranche-v1.json`, `src/data/research/national-acquisition-plans/usgs-bbs-co-route-start-tranche-v1.json`, `src/data/research/runs/20260828T030700Z__usgs-bbs__f2aa993e6e33/`, `src/data/research/runs/20260828T061928Z__usgs-bbs__154ba0c33ef3/`, `src/data/research/runs/20260829T001541Z__usgs-bbs__5e939aaf0e89/`, `src/data/research/runs/20260829T013803Z__usgs-bbs__eb398d7969fa/`, `src/data/research/runs/20260829T042826Z__usgs-bbs__4b960d0a5849/`, `public/generated/research/TX/`, `public/generated/research/IL/`, `public/generated/research/IN/`, `public/generated/research/CA/`, `public/generated/research/CO/` | The legacy Alabama importer and registered `usgs-bbs-route-start@1.1.1` adapter verify the 2026 ScienceBase release identity plus exact byte counts and MD5 checksums for `Routes.csv`, `SpeciesList.csv`, `Weather.csv`, and `50-StopData.zip`; require exact current-catalog binomials and exact BBS AOU matches; filter to United States `RunType=1` standard runs with positive `Stop1` counts; and require route-start coordinates to resolve inside exactly one committed active county. The complete national scan reviewed `10` catalog bird species, found `7` exact BBS matches, resolved `4,453` of `4,458` United States routes, screened `4,551,591` archive rows, retained `5,322` positive standard-run Stop 1 rows, and identified `1,548` gross / `1,431` net county-species pairs before the new state tranches. The Texas tranche retained `426` rows and moved `116` unique pairs from not researched to verified present. A post-Texas rescan measured `1,315` remaining net pairs; the Illinois tranche retained `388` rows and moved `87` more unique pairs across `48` counties (`44` House Sparrow, `29` European Starling, `11` Eurasian Collared-Dove, `2` Rock Pigeon, and `1` Mute Swan). A post-Illinois rescan measured `1,228` remaining net pairs; Indiana and California tied at `69`, and the smaller modeled Indiana footprint was selected. Indiana retained `254` supporting rows and moved `69` unique pairs across `41` counties (`38` House Sparrow, `24` European Starling, and `7` Rock Pigeon). California then retained `292` supporting rows and moved `69` unique pairs across `34` counties (`28` House Sparrow, `22` European Starling, `18` Eurasian Collared-Dove, and `1` Rock Pigeon). A post-California rescan measured `1,090` remaining net pairs; Colorado and Ohio tied at `59`, and Colorado was selected because all `59` gross pairs were net-new and its `231` supporting rows were fewer than Ohio's `255`. Colorado moved `59` unique pairs across `32` counties (`24` House Sparrow, `17` Eurasian Collared-Dove, `16` European Starling, and `2` Rock Pigeon). All assertions carry topology and coordinate-set receipts. The earlier Alabama pilot retained `77` rows and produced `37` gross / `0` net pairs. Treat every BBS assertion as sampled point presence only, never countywide presence, later-stop county evidence, route-total evidence, non-detection, absence, establishment proof, or complete bird inventory coverage. | Alabama accessed `2026-06-16`; current release, Texas tranche, and Illinois tranche accessed `2026-08-28` UTC; Indiana, California, and Colorado tranches accessed `2026-08-29` UTC |
| iDigBio Search API, https://search.idigbio.org/v2/search/records | County-level Alabama physical occurrence support from exact catalog scientific-name matches with preserved specimen records | `src/data/source/idigbio-alabama-preserved-specimens-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-idigbio-alabama-specimens.ts` queries public iDigBio Alabama `preservedspecimen` records for exact generated catalog names, accepts explicit Alabama county values or coordinates that resolve to one Alabama county, filters cultivated-looking localities, and `scripts/import-county-presence.ts` merges the resulting snapshot. Treat this as verified physical occurrence evidence, not proof of established invasive population or absence. | Accessed and merged on `2026-04-25` |
| Water Quality Portal biological results, https://www.waterqualitydata.us/data/Result/search | County-level Alabama biological monitoring result rows for exact current-catalog aquatic taxa | `src/data/source/wqp-alabama-biological-results-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:wqp-alabama` queries the federal cooperative WQP Result service with `statecode=US:01`, `dataProfile=biological`, `sampleMedia=Biological`, and exact `subjectTaxonomicName` values. The pilot accepts only `ActivityMediaName=Biological`, exact `SubjectTaxonomicName`, blank `UnidentifiedSpeciesIdentifier`, blank `ResultDetectionConditionText`, `CharacteristicName=Count`, positive numeric count values, `Final` or `Accepted` result status, and point coordinates resolving to one Alabama county. The pilot imported `4` species from `8` accepted count records and `7` gross county-species source pairs, adding `5` net Alabama matrix determinations. Broader `characteristicName` or `characteristicType` probes were not used because they returned measurement rows rather than organism occurrence rows. Treat this as biological monitoring presence evidence, not survey non-detection, countywide absence, or complete aquatic inventory coverage. | Accessed and merged on `2026-06-15` |
| iNaturalist Research Grade observations, https://api.inaturalist.org/v1/observations | Supplemental Alabama county-level observed-presence support for a bounded exact-species pilot | `src/data/source/inaturalist-alabama-research-grade-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:inaturalist-alabama` queries the public iNaturalist API for an expanded bounded pilot set of exact active species taxa in Alabama `place_id=19`, requires `quality_grade=research`, `captive=false`, `mappable=true`, non-obscured public coordinates, `taxon_geoprivacy` and `geoprivacy` values of `open` or null, and point-to-county resolution against local Alabama county geometry. For bounded follow-ups, `INATURALIST_TARGETS` can refresh only named exact targets while preserving prior source rows. The source audit snapshot stores accepted observation IDs, URLs, observer labels, dates, coordinates, geoprivacy flags, and county FIPS. The current snapshot now tracks `688` exact current-catalog targets, imports `672` species, stores `68484` accepted county-resolved observations, and produces `8614` gross Alabama county-species source pairs. The 2026-07-06 targeted follow-up reviewed `11` additional exact active taxa (`Solanum nigrescens`, `Hemidactylus garnotii`, `Mentha pulegium`, `Kalanchoe delagoensis`, `Cryptomeria japonica`, `Festuca rubra`, `Poa trivialis`, `Lasioderma serricorne`, `Phleum pratense`, `Fallopia convolvulus`, and `Digitaria violascens`) but found `0` strict Research Grade API observations under the project filters, so it added `0` county-species source pairs and `0` matrix determinations. Treat this as supplemental community-verified observed presence, not agency survey evidence, establishment proof, invasive-impact proof, non-detection, or absence. | Accessed and merged on `2026-07-06` |
| USFWS invasive carp eDNA sample layer, https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest/services/USFWS_Great_Lakes_Region_3_Bighead_and_Silver_Carp_eDNA_Data_Feature_Layer_View/FeatureServer/0 | Survey-area non-detection evidence for Alabama bighead carp and silver carp county-species matrix records | `src/data/source/county-species-status-overrides.ts`, `docs/county-coverage/states/AL.json`, `docs/county-coverage/states/AL.md` | Public ArcGIS FeatureServer sample rows were reviewed for Alabama `No eDNA detected` records. Only county-species pairs without conflicting current verified-present or positive eDNA evidence are retained as `not-detected` with `survey-area` evidence scope. The 2026-07-11 NAS import superseded Bighead Carp non-detections in Colbert and Jackson counties, leaving `6` current eDNA non-detection records. | Accessed and reviewed on `2026-04-25`; conflict rechecked on `2026-07-11` |
| USDA APHIS National Honey Bee Survey, https://www.usbeedata.org/state_reports/public_download/ | County-level Alabama sample evidence for honey bee mite, plus conservative sample-level non-detection records | `src/data/source/county-presence-snapshot.json`, `src/data/source/county-species-status-overrides.ts`, `src/data/generated/presence.json`, `public/generated/presence.json`, `docs/county-coverage/states/AL.json`, `docs/county-coverage/states/AL.md` | `scripts/import-aphis-honey-bee-survey.ts` fetches the current public signed CSV from the download page, imports Alabama rows with `varroa_per_100_bees > 0` as `Varroa destructor` verified-present county sample evidence, skips malformed or unresolved counties such as `Scott`, and leaves zero-count rows out of presence. Clarke and Sumter zero-count rows are recorded as `not-detected` with `survey-area` evidence scope because no positive row for those counties exists in the reviewed file. Treat this as sample evidence, not countywide absence. | Accessed and merged on `2026-06-14`; source file generated by US Bee Data on `2026-06-05` and includes collection dates through `2025-12-31` |
...
| https://www.forestry.alabama.gov/Pages/News/2025/Emerald_Ash.aspx | Alabama county audit; Emerald Ash Borer | Official Alabama Forestry Commission emerald ash borer confirmation used for county-detail evidence and manual county-presence overrides, including additive Etowah and Shelby County evidence beyond the current APHIS layer |
...
| https://nas.er.usgs.gov/queries/CollectionInfo.aspx?SpeciesID=463 | Alabama NAS live collection supplement; Blue Tilapia | Live NAS collection rows used for targeted Alabama ANS gap review. Lee County was accepted from an established row, while failed and no-county rows were skipped. |
| https://nas.er.usgs.gov/queries/CollectionInfo.aspx?SpeciesID=3294 | Alabama NAS live collection supplement; Koi | Live NAS collection rows used for targeted Alabama ANS gap review. Madison and Mobile County rows were accepted as collection presence, not establishment evidence. |
| http://floraofalabama.org/ | Alabama Plant Atlas county import | County distribution maps from vouchered Alabama Herbarium Consortium records used for the 2026-04-25 Alabama Plant Atlas source-family imports |
| https://dev.alabama.plantatlas.usf.edu/api/services/app/PlantSpecies/SearchSciName | Alabama Plant Atlas county import | Public scientific-name API used for exact catalog plant matching before importing Alabama county FIPS distribution rows |
| https://dev.alabama.plantatlas.usf.edu/api/services/app/PlantSpecies/GetDistributionFIPSForSpecies | Alabama Plant Atlas county import | Public county FIPS API used for exact Alabama Plant Atlas species matches |
| https://sernecportal.org/portal/collections/individual/index.php?occid=14386315 | Alabama county audit; Australian-pine | Exact `Casuarina equisetifolia` Baldwin County NCU herbarium record used for manual county-presence override |
| https://sernecportal.org/portal/collections/individual/index.php?occid=14386322 | Alabama county audit; Australian-pine | Second exact `Casuarina equisetifolia` Baldwin County NCU herbarium record used to support the manual county-presence override |
| https://research.fs.usda.gov/nrs/products/dataandtools/alien-forest-pest-explorer | Alabama county audit; USFS Alien Forest Pest Explorer | Official USFS tool description for the forest pest and pathogen source family |
| https://purr.purdue.edu/publications/4479/1 | Alabama county audit; USFS Alien Forest Pest Explorer | PURR dataset publication supporting the `USFS Alien Forest Pest Explorer` county importer |
| https://www.arcgis.com/home/item.html?id=c066471a25394701b698552e945f2088 | Alabama county audit; USFS Current Invasive Plant Locations | ArcGIS item describing the Alabama Current Invasive Plants web map filtered to National Forest System land in Alabama |
| https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_InvasiveSpecies_01/MapServer/0 | Alabama county audit; USFS Current Invasive Plant Locations | Public EDW FeatureServer layer used for current invasive plant infestation polygons on National Forest System land |
| https://agi.alabama.gov/plantprotection/2025/11/citrus-canker-detected-in-mobile-county/ | Alabama county audit; Citrus canker | Official Mobile County citrus canker detection added to the existing citrus canker manual county-presence override |
| https://api.gbif.org/v1/species/match | Alabama GBIF specimen import | Public GBIF backbone endpoint used for strict exact scientific-name matching before Alabama preserved specimen occurrence import |
| https://api.gbif.org/v1/occurrence/search | Alabama GBIF specimen import | Public GBIF occurrence API used for Alabama `PRESERVED_SPECIMEN` records with explicit county values |
| https://www.gbif.org/occurrence/search | Alabama GBIF specimen import | Human-readable GBIF occurrence search URL carried in county source references |
| https://services7.arcgis.com/2C1NQ7u6M6SXoa8p/arcgis/rest/services/PPQ_GIS_Federal_Quarantine_AGOL_EDIT_Feature_Layer_view/FeatureServer/1 | Alabama APHIS federal quarantine import | Public APHIS PPQ Federal Quarantine county layer used for active Alabama quarantine rows matching reviewed catalog pest and pathogen programs |
| https://search.idigbio.org/v2/search/records | Alabama iDigBio specimen import | Public iDigBio Search API used for Alabama preserved specimen records with explicit county values or coordinates that resolve to Alabama counties |
| https://portal.idigbio.org/portal/search | Alabama iDigBio specimen import | Human-readable iDigBio portal search URL carried in county source references |
| https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest/services/USFWS_Great_Lakes_Region_3_Bighead_and_Silver_Carp_eDNA_Data_Feature_Layer_View/FeatureServer/0 | Alabama USFWS eDNA non-detection review | Public USFWS invasive carp eDNA sample layer used for conservative `not-detected` survey-area records for bighead carp and silver carp |
| https://www.pesttracker.org/pests | Alabama survey-status source review | Public PestTracker pest list reviewed for mapped catalog species. Current map rows exposed Alabama county FIPS rows for some pests, but no exact current-catalog Alabama row matches were found in the reviewed output |
| https://caps.ceris.purdue.edu/napis-data-definitions-2/ | Alabama survey-status source review | CAPS and NAPIS status-code definitions confirming that public survey data can distinguish positive and negative survey records where raw rows are accessible |
| https://www.usbeedata.org/state_reports/public_download/ | Alabama survey-status source review | APHIS National Honey Bee Survey public CSV reviewed for Alabama sample records. The current public file exposes county-resolved Alabama rows via `sampling_county_from_gps`; `Varroa destructor` was imported for positive county sample evidence, and zero-count Clarke and Sumter sample rows were added only as survey-area `not-detected` records |
