...
| Alabama Plant Atlas, http://floraofalabama.org/ | County-level Alabama plant occurrence support from vouchered Alabama Herbarium Consortium distribution maps | `src/data/source/state-species-denominators.ts`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-county-presence.ts` queries Alabama Plant Atlas species pages for matched ALIPC denominator species, including reviewed accepted-name aliases such as `Camphora officinarum` for `Cinnamomum camphora` and `Dioscorea polystachya` for `Dioscorea oppositifolia` | Accessed and merged on `2026-04-25` |
...
| http://floraofalabama.org/ | Alabama Plant Atlas county import | County distribution maps from vouchered Alabama Herbarium Consortium records used for the 2026-04-25 Alabama Plant Atlas source-family import |
