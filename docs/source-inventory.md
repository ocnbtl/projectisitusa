...
| iNaturalist Research Grade observations, https://api.inaturalist.org/v1/observations | Supplemental Alabama county-level observed-presence support for a bounded exact-species pilot | `src/data/source/inaturalist-alabama-research-grade-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:inaturalist-alabama` queries the public iNaturalist API for an expanded bounded pilot set of `376` exact active species taxa in Alabama `place_id=19`, requires `quality_grade=research`, `captive=false`, `mappable=true`, non-obscured public coordinates, `taxon_geoprivacy` and `geoprivacy` values of `open` or null, and point-to-county resolution against local Alabama county geometry. The source audit snapshot stores accepted observation IDs, URLs, observer labels, dates, coordinates, geoprivacy flags, and county FIPS. The latest 2026-06-27 expansion reviewed `376` exact current-catalog targets, found `376` exact active iNaturalist taxa, reviewed `68219` strict API observations, accepted `67060` county-resolved observations, imported `375` species, produced `7818` gross Alabama county-species source pairs, reported `392` net new source-snapshot county-species pairs, and raised the regenerated Alabama matrix by `391` verified-present determinations after generated unioning. `Dichrostachys cinerea` had strict API matches but no county-resolved accepted rows, so it remains without imported county evidence from this source. Domestic Cat, Mute Swan, Koi, obvious crop records, and other high-ambiguity taxa remain excluded from this source alone; recent exclusions include additional crop or domestic taxa such as `Felis catus`, `Cygnus olor`, `Cyprinus rubrofuscus`, `Cucumis melo`, `Triticum aestivum`, `Solanum lycopersicum`, `Solanum tuberosum`, `Ipomoea batatas`, `Sorghum bicolor`, `Citrullus lanatus`, `Avena sativa`, `Prunus persica`, `Oryctolagus cuniculus`, and `Cocos nucifera`. Treat this as supplemental community-verified observed presence, not agency survey evidence, establishment proof, invasive-impact proof, non-detection, or absence. | Accessed and merged on `2026-06-27` |
