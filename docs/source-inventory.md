...
| USGS North American Breeding Bird Survey, DOI: https://doi.org/10.5066/P144YU3S | Official Alabama route-start survey detection support for exact current-catalog bird taxa | `src/data/source/bbs-alabama-route-start-birds-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:bbs-alabama` downloads the 2026 ScienceBase release metadata plus `Routes.csv`, `SpeciesList.csv`, `Weather.csv`, and `50-StopData.zip`, requires exact current catalog species names and exact BBS species-list matches, filters to United States Alabama routes, accepts only `RunType=1` standard runs with positive `Stop1` counts, and resolves route-start coordinates to one Alabama county. The pilot reviewed `8` exact current-catalog bird species, found `7` exact BBS species-list matches, imported `4` species from `77` accepted Stop 1 detections, produced `37` gross county-species source pairs, and added `0` net Alabama matrix determinations because those county-species pairs were already present from other sources. Treat BBS rows as official route-start survey presence evidence only, not countywide presence, later-stop county evidence, route-total evidence, non-detection, absence, establishment proof, or complete bird inventory coverage. | Accessed and merged on `2026-06-16` |
