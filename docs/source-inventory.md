...
| USGS NAS live collection pages, https://nas.er.usgs.gov/queries/CollectionInfo.aspx | Targeted county-level occurrence support for reviewed Alabama ANS denominator gaps not covered by the local NAS archive import | `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-county-presence.ts` parses reviewed live CollectionInfo pages by NAS species ID, accepts Alabama county rows, skips failed, state-only, no-county, and no-Alabama rows, and applies species-specific status filters where needed | Accessed and merged on `2026-04-26` |
...
| iNaturalist Research Grade observations, https://api.inaturalist.org/v1/observations | Supplemental Alabama county-level observed-presence support for a bounded exact-species pilot | `src/data/source/inaturalist-alabama-research-grade-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:inaturalist-alabama` queries the public iNaturalist API for an expanded bounded pilot set of exact active species taxa in Alabama `place_id=19`, requires `quality_grade=research`, `captive=false`, `mappable=true`, non-obscured public coordinates, `taxon_geoprivacy` and `geoprivacy` values of `open` or null, and point-to-county resolution against local Alabama county geometry. The source audit snapshot stores accepted observation IDs, URLs, observer labels, dates, coordinates, geoprivacy flags, and county FIPS. The 2026-06-29 exact live-discovery expansion reviewed `677` exact current-catalog targets, found `677` exact active iNaturalist taxa, reviewed `69679` strict API observations, accepted `68484` county-resolved observations, imported `672` species, produced `8614` gross Alabama county-species source pairs, and added `230` net Alabama matrix determinations after generated unioning. The newest `75` target tranche imported county evidence for `74` species, produced `328` gross county-species pairs, and added `230` net matrix determinations across `63` species; `Oryctolagus cuniculus` had strict API matches but no county-resolved accepted rows, so it remains without imported county evidence from this source. Prior no-county targets `Dichrostachys cinerea`, `Charybdis hellerii`, `Diplosoma listerianum`, and `Eragrostis cumingii` still lack county-resolved accepted rows from this source. Contradiction resolved from older notes: the current bounded iNaturalist target set now includes previously excluded exact live-discovered domestic, crop, and other high-ambiguity catalog taxa, but only as supplemental observed-presence records under the strict iNaturalist filters. Treat this as supplemental community-verified observed presence, not agency survey evidence, establishment proof, invasive-impact proof, non-detection, or absence. | Accessed and merged on `2026-06-29` |
| USFWS invasive carp eDNA sample layer, https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest/services/USFWS_Great_Lakes_Region_3_Bighead_and_Silver_Carp_eDNA_Data_Feature_Layer_View/FeatureServer/0 | Survey-area non-detection evidence for Alabama bighead carp and silver carp county-species matrix records | `src/data/source/county-species-status-overrides.ts`, `docs/county-coverage/states/AL.json`, `docs/county-coverage/states/AL.md` | Public ArcGIS FeatureServer sample rows were reviewed for Alabama `No eDNA detected` records. Only county-species pairs without conflicting current verified-present or positive eDNA evidence were added as `not-detected` with `survey-area` evidence scope. | Accessed and reviewed on `2026-04-25` |
