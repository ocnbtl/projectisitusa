...
| --- | --- | --- | --- | --- |
| Official Alabama, Alaska, Arkansas, and Arizona invasive or regulated species lists | State-level regulated applicability only, separate from county evidence and source-screen completion | `src/data/research/state-applicability-source-registry.json`, `src/data/research/state-list-sources/`, `src/data/research/state-applicability/AL.json`, `src/data/research/state-applicability/AK.json`, `src/data/research/state-applicability/AR.json` | Official Alabama and Alaska PDFs and the Arkansas rule HTML were retained with exact hashes and bytes. `npm run research:apply:state-lists` validates their descriptors, allows only exact canonical catalog binomials, and merges reviewed bases deterministically. The first review accepts `49` events and preserves `22` ambiguous or unmatched rows as blocked. Arizona remains registered as blocked because official automated acquisition returned provider challenge pages. List membership establishes state regulatory applicability only and never county occurrence, absence, non-detection, or not-applicability. | Retrieved and reviewed on `2026-07-28` |
