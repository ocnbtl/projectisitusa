...
| iNaturalist Research Grade observations, https://api.inaturalist.org/v1/observations | Supplemental Alabama county-level observed-presence support for a bounded exact-species pilot | `src/data/source/inaturalist-alabama-research-grade-snapshot.json`, `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `npm run import:inaturalist-alabama` queries the public iNaturalist API for an expanded bounded pilot set of `602` exact active species taxa in Alabama `place_id=19`, requires `quality_grade=research`, `captive=false`, `mappable=true`, non-obscured public coordinates, `taxon_geoprivacy` and `geoprivacy` values of `open` or null, and point-to-county resolution against local Alabama county geometry. The source audit snapshot stores accepted observation IDs, URLs, observer labels, dates, coordinates, geoprivacy flags, and county FIPS. The 2026-06-28 low-count expansion reviewed `602` exact current-catalog targets, found `602` exact active iNaturalist taxa, reviewed `68847` strict API observations, accepted `67671` county-resolved observations, imported `598` species, produced `8285` gross Alabama county-species source pairs, and added `136` net Alabama matrix determinations after generated unioning. `Dichrostachys cinerea`, `Charybdis hellerii`, `Diplosoma listerianum`, and `Eragrostis cumingii` had strict API matches but no county-resolved accepted rows, so they remain without imported county evidence from this source. Domestic Cat, Mute Swan, Koi, obvious crop records, and other high-ambiguity taxa remain excluded from this source alone; recent exclusions include additional crop or domestic taxa such as `Felis catus`, `Cygnus olor`, `Cyprinus rubrofuscus`, `Cucumis melo`, `Triticum aestivum`, `Solanum lycopersicum`, `Solanum tuberosum`, `Ipomoea batatas`, `Sorghum bicolor`, `Citrullus lanatus`, `Avena sativa`, `Prunus persica`, `Oryctolagus cuniculus`, `Cocos nucifera`, `Coriandrum sativum`, `Medicago sativa`, and `Secale cereale`. Treat this as supplemental community-verified observed presence, not agency survey evidence, establishment proof, invasive-impact proof, non-detection, or absence. | Accessed and merged on `2026-06-28` |
