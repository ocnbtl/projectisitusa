...
| USDA Forest Service EDW Current Invasive Plant Locations, https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_InvasiveSpecies_01/MapServer/0 | Current Alabama National Forest invasive plant infestation polygon support for exact catalog species | `src/data/source/county-presence-snapshot.json`, `src/data/generated/presence.json`, `public/generated/presence.json` | `scripts/import-county-presence.ts` queries the public EDW FeatureServer layer filtered to `NFS IN ALABAMA NATIONAL FOREST`, matches exact accepted scientific names to catalog targets, resolves each infestation polygon by bbox center to an Alabama county, and skips generic genus-level or non-catalog plant names | Accessed and merged on `2026-04-26` |
