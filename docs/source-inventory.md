...
| --- | --- | --- | --- | --- |
| Official Alabama, Alaska, Arkansas, and Arizona invasive or regulated species lists | State-level regulated applicability only, separate from county evidence and source-screen completion | `src/data/research/state-applicability-source-registry.json`, `src/data/research/state-list-sources/`, `src/data/research/state-applicability/AL.json`, `src/data/research/state-applicability/AK.json`, `src/data/research/state-applicability/AR.json`, `src/data/research/state-applicability/AZ.json` | Official Alabama and Alaska PDFs and the Arkansas rule HTML were retained with exact hashes and bytes. The official Arizona Secretary of State Title 3 Chapter 4 PDF was browser-downloaded because direct non-browser requests remained challenge-blocked, then retained at `2,738,828` bytes with SHA-256 `b49c4428f1182fbe458de2d6219c4694aa556de617d212db2fec18c1a4347308`. `npm run research:apply:state-lists` validates descriptors, allows only exact canonical catalog binomials, and merges reviewed bases deterministically. The Arizona R3-4-245 review covers all `60` rows in Tables 4-6: `40` accepted events across `39` species and `20` blocked rows. List membership establishes state regulatory applicability only and never county occurrence, absence, non-detection, or not-applicability. | Alabama, Alaska, and Arkansas retrieved and reviewed on `2026-07-28`; Arizona retrieved and reviewed on `2026-08-04` from Supplement 25-1 dated `2025-03-31` |
