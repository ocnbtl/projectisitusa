# Alabama County Audit Chunk 001

## Scope

This is the first Alabama county-audit chunk. It reviews the first `5` counties in the state checklist:
...
- Alabama counties: `67` total, `5` completed, `7.46%`
- Alabama chunks: `14` total, `1` completed, `7.14%`
- Nationwide counties in current 50-state audit set: `3141` total, `5` completed, `0.16%`
- Nationwide chunks at `5` counties each: `629` total, `1` completed, `0.16%`

## Recommendation

1. Continue Alabama sequentially so the audit math stays simple and reproducible.
2. Treat county-specific official reports as useful evidence, but keep calling out when they are only single-species or single-incident reports.
3. Finish the next Alabama chunk before widening to another state, so the process and source thresholds stay consistent.
