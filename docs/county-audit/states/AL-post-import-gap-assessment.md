...
- National generated catalog: `2504` species, with `1606` mapped and `898` unmatched.
...
| Alabama Invasive Plant Council list, mirrored by Invasive Plant Atlas and EDDMapS | `91` plant species reported invasive in Alabama natural areas | Partly importable through EDDMapS subject county records and Alabama Plant Atlas county maps | Strong plant denominator. Current generated reconciliation maps `75` ALIPC catalog-matched species from county-level EDDMapS, Alabama Plant Atlas, SERNEC, NAS, AFC, specimen, or prior rows. List membership alone remains insufficient for county production mapping. |
...
| ALIPC catalog-matched but unmapped species | `lonicera-x-bella`, `callicarpa-japonica`, `ardisia-japonica` | Alabama Plant Atlas recheck, SERNEC expansion, EDDMapS recheck, iDigBio review, or source-specific county records | Remaining plant follow-up after the ALIPC-backed EDDMapS, Alabama Plant Atlas, SERNEC, GBIF, and iDigBio passes. Earlier notes incorrectly treated `ardisia-japonica` as live mapped and kept `solanum-tampicense` in the unmapped bucket. Current generated presence shows `ardisia-japonica` has no Alabama county rows and `solanum-tampicense` is live mapped in Wilcox County. Near matches remain excluded without reviewed species-level evidence. |
...
| Category balance | Alabama still has thin insect, wildlife, pathogen, and disease coverage relative to plants and aquatic species, though GBIF, APHIS, USFS, and APHIS National Honey Bee Survey improved recent passes | APHIS, ADAI, USFS, AFC forest-health layers, state wildlife sources, CAPS or PestTracker survey data, and state pest notices | High priority for future source-family work, especially for sources that encode surveyed-and-not-found statuses. APHIS National Honey Bee Survey is now county-importable for Alabama `Varroa destructor` sample evidence; PestTracker remains unavailable through the reviewed public path. |
...
- A 2026-04-26 gap note incorrectly said `ardisia-japonica` was live mapped from iDigBio and kept `solanum-tampicense` in the ALIPC unmapped bucket. Current generated data shows the opposite for those two items: `ardisia-japonica` remains unmapped in Alabama, while `solanum-tampicense` is live mapped in Wilcox County.
...
6. Survey-status source lane.
   - Status: first USFWS invasive carp eDNA `not-detected` records are added with `survey-area` evidence scope. APHIS National Honey Bee Survey now adds county-resolved Alabama Varroa positive sample evidence and two conservative zero-count sample non-detections. PestTracker public map output still did not expose Alabama county rows for the reviewed mapped pests.
