...
Runtime data generated after the ALIPC EDDMapS import, generalized Alabama Plant Atlas import, AFPE forest-pest importer, GBIF preserved specimen snapshot, and APHIS federal quarantine importer shows:

- Alabama county audit progress: `67/67` counties and `14/14` chunks complete.
- Alabama live mapped species: `753` distinct species.
- Alabama average county species count: `152.9`.
- Alabama county-species matrix known determinations: `10244` verified present, `0` verified absent, `0` not detected, and `157524` unknown.
- Alabama county-species matrix known percent: `6.11%` of `167768` county-species determinations.
- Alabama category mix: still plant-heavy, but recent passes added broader insect, disease, wildlife, and specimen-backed occurrence signal from USFS Alien Forest Pest Explorer, GBIF, and APHIS.
...
- National generated catalog: `2504` species, with `1480` mapped and `1024` unmatched.

This is a major improvement from the earlier Alabama coverage gap, but it does not prove that Project Isitusa maps 90 percent or more of every invasive species present in Alabama. The attempted 10 percent knowledge milestone was not reached in this pass because the newly added source families support verified-present records only. They do not support broad verified-absent or not-detected determinations.
...
- Physical specimen occurrence records from GBIF exact catalog matches, especially in counties with universities, museums, and herbarium collection history. These improve verified-present coverage but need source-language caution because specimen occurrence is not the same thing as a county invasive-status declaration.
...
4. GBIF and museum-portal refinement.
   - Status: broad GBIF preserved specimen import is now available as a repeatable snapshot.
   - Next goal: review whether dataset allowlists, iDigBio, SERNEC expansion, or institution-scoped GBIF downloads can strengthen the broad specimen lane and reduce cultivated or non-established noise.
